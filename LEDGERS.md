...
maintained spine divergence remains. The restored owner-authority gate (issue #482) —
retired in issue #1264 (C5 merge-gate retirement); record in §1.2 — is no longer a
maintained divergence. Maintained non-spine divergences now begin with review-code's
multi-model review panel (issue #513, from the ratified #474 design, the S2 Review-Crew-v2
lane) — a fresh B6 analysis, not authored on the spine's exit.

### 1.1 Maintained divergences

| Mechanism | Platform primitive that could absorb it | Why we still diverge | Re-check trigger |
|---|---|---|---|
| **Worktree guard (minimal PreToolUse Bash hook, issue #682)** — emits `permissionDecision: "deny"` when a classified git command would irrecoverably discard uncommitted worktree content — `git checkout` that reads paths (a `--` pathspec, `--pathspec-from-file`, two or more operands, or a lone operand that names an existing path, does not resolve as a commit, or cannot be resolved at all: fail-closed), `git checkout -f`/`--force`, `git switch -f`/`--force`/`--discard-changes`, `git restore` except index-only `--staged`, `git reset --hard`, any `git clean` that is not a dry run, `git checkout-index -f`/`--force`, `git rm -f`/`--force`, and `git worktree remove -f`/`--force` — scoped to superheroes-calibrated projects; wired on Claude Code's `hooks/hooks.json` — **Codex could carry this floor**; no consuming project has asked for it, so it is not wired there; the checkout-revert wipe class — a mutation probe or fix subagent reverting with `git checkout -- <file>` while a prior order's uncommitted delivery sits at that path | Plugin-shippable native permission rules — declarative deny/ask sets a plugin can carry — but the platform's permission model has no way for a plugin to express *deny conditionally, based on live repository state*: a static allow/deny/ask set cannot consult `git status`, so even plugin-shippable native permission rules would not, on their own, absorb this one | An owner-named risk with eight recorded in-repo occurrences (plus five in a sibling project) needs a mechanical tripwire, not prose alone (PHILOSOPHY B6); prose demonstrably failed — twice with the warning already loaded in the session's context and the charter's prose rule in force | A platform primitive that lets a plugin express a **state-conditional** deny → move the guard onto it and retire the hook; OR Codex's PreToolUse payload shape is measured and the gate can be wired on Codex without a wrong guess shipping a silently-inert hook; OR the refusals turn out in practice to be mostly false positives → revisit the dirtiness heuristic. *False-positive specimen tally (kept here so that trigger can ever be evaluated):* **1** — 2026-08-06, harness 2.1.222, a shell heredoc whose payload quoted git commands as prose (issue #905, closed with tripwire; the refusal message has hinted at this shape since PR #875) |
| **Bash timeout floor (`hooks/bash_timeout.py`, second PreToolUse(Bash) hook in `plugins/superheroes/hooks/hooks.json`)** — when a Bash call omits an explicit `timeout`, rewrites the tool input via `hookSpecificOutput.updatedInput` to inject a 600000ms floor (matching `verify_gate.py`'s default) so long foreground dispatches are not killed at the harness's 120s default; an explicit model-passed timeout is never touched; fail-open on parse/shape errors | A host-configurable per-command timeout the plugin could declare rather than hook | Host-specific mechanical floor; **Codex could carry it** and no consuming project has asked, so the absence there is disclosed, not a host incapability | The host ships a declarative per-command timeout the plugin can set, OR a consuming project asks for the floor on Codex |
| **Multi-model review panel (review-code's multi-model review loop)** — the five-lens cross-vendor panel + fix loop + durable receipts, shipped as the review-code v2 arc (#505–#513, design #474) | Anthropic's upstream code-review surfaces — the public code-review plugin and the built-in `/code-review` skill. The #474 design deliberately adopted three of their patterns where they CONVERGED with ours: per-finding verification with a CONFIRMED/PLAUSIBLE/REFUTED verdict ladder requiring quoted evidence, an explicit do-not-flag list, and a no-silent-drops final merge | Capability 3 — durable receipts posted on the PR — is **absorbed upstream** (recorded 2026-08-06, #318 pass 1, re-walked here per #902: Managed Code Review posts inline comments, a check run carrying a per-finding severity table, Files-changed annotations, and a machine-readable `bughunter-severity` payload; local `/code-review --comment` posts inline comments). The divergence now stands on capabilities 1, 2, and the remainder of 4: multi-round fix loops with fresh-eyes fix audits; cross-vendor seats (family-keyed independence — the B3 rung; upstream is multi-agent, single-vendor); and park adjudication, circuit breakers, and the owner-facing halt-with-open-blocker path (upstream's absorbed half of 4 stops at named error/timeout check-run titles and a spend-cap comment). Those three capabilities are the product's spine — PHILOSOPHY promises 4/6 — not incidental extras a single-pass code-review skill could stand in for. Availability and economics carry secondary weight (not spine, and not re-check triggers): Managed Code Review is Team/Enterprise-only, unavailable under Zero Data Retention, ~$15–25 per review; the local `/code-review` posts no check run and no machine-readable receipt | An upstream release absorbing any of the REMAINING capabilities — 1, 2, or the park/breaker half of 4 — → re-walk this row (the orientation review, #318, owns the cadence). Riding #318 pass 2, recorded open: whether the upstream check-run severity payload belongs inside our loop as an input rather than a competitor. Logged: the upstream low/medium/high effort-cells mechanic was evaluated and DEFERRED as adoptable-later (#474 position 14) — delta rounds fixed the cost curve instead |
...
| **Headless builder launcher (`rubric/launch-doctrine.md`, `lib/launch_doctrine.py`, `lib/launch_ledger.py`, `lib/launcher.py`, issue #656)** — composes an advisor-invoked headless builder launch from fail-closed versioned doctrine, stamps the dispatch premise, keeps an append-only durable dispatch record, and runs R1's mechanical park/refusal accounting. Named consumer: the Showrunner charter's orchestration duty — `skills/showrunner/SKILL.md` §9 dispatch preflight; the advisor invokes the launcher per launch | Nearest primitive today is **`claude --bg`** running under a per-`CLAUDE_CONFIG_DIR` supervisor, plus `SendMessage notify_when_idle` (research preview; sessions survive sleep but stop on machine shutdown; the idle notice is one-shot with a twelve-hour expiry; waking across instances leans on the shared sessions symlink). Also nearest in weaker senses: the host's subagent/Task spawning and `lib/engine_dispatch.py` (external-engine dispatches with a supervisor journal and confinement tripwires, evaluated for review/write seats). None composes a headless *builder* session from a versioned fail-closed doctrine artifact, stamps enumerated grant scope and owner-capability premises at reserve time, or keeps an interprocess-locked append-only launch ledger whose `count` refuses a batch rate when the fold is incomplete | Ratified precondition: #526 seat ruling B (owner-settled 2026-07-26, LEDGERS.md §4) — *launch shape becomes scripted so nothing is reconstructed from memory*. Evidence: the shared-checkout collision (a rulings block rebuilt from memory dropped the own-worktree line; two builders shared the primary checkout and a sibling's uncommitted work was wiped), three corpus launch failures behind the bounded retry, and R1's sentence making the mechanical count a blocking deliverable of this build. **§2 boundary:** advisor-invoked, one launch at a time — not queue machinery, not board-driven automatic dispatch; does not partial-unlock the #526 "No queue machinery ahead of the build-dispatch discovery" entry. **Background-session trial (2026-09-15, §5):** the re-check trigger fired; the re-check ran; the outcome is **keep** — see §5.5 (receipt not clean; nothing retires on this receipt) | The host ships a native headless-launch primitive that carries versioned standing rulings, enumerated premise stamping, and durable per-batch dispatch accounting the advisor can invoke without bespoke modules → the orientation review re-walks this row and retires the launcher stack; OR sustained field evidence that manual launch discipline plus ledgered accounting has had zero doctrine-parse, premise-stamp, or overlap escapes across measured multi-build waves → revisit whether the divergence still earns its keep. Past re-walk (#903): **agent teams** were walked and found not to be the nearest primitive (experimental, in-process only, cannot outlive the lead, absent entirely under `-p`); shared verbatim with the heartbeat row |
| **Builder liveness heartbeat + advisor sweep (`lib/heartbeat.py`, launcher lane-id/root export via `SUPERHEROES_LAUNCH_ID` / `SUPERHEROES_HEARTBEAT_ROOT`, showrunner wave sweep duty)** — the builder stamps lane endings and blockers; the advisor's scheduled sweep runs `heartbeat.py sweep` and acts on `terminal` / `nonterminal` / `unknown`, paired with `wave_watch.py run` for liveness (`lane-stale`). Named consumer: the Showrunner charter's orchestration duty 9 scheduled liveness sweep — `skills/showrunner/SKILL.md` duty 9 | Nearest primitive today is **`claude --bg`** under a per-`CLAUDE_CONFIG_DIR` supervisor, plus `SendMessage notify_when_idle` (research preview; sessions survive sleep but stop on machine shutdown; the idle notice is one-shot with a twelve-hour expiry; waking across instances leans on the shared sessions symlink). Weaker: the host's background-task completion notification — on harness 2.1.219 completions reach the **root session**, not the spawning builder, so wake-on-completion does not reliably resume the lane that launched the work; the 2.1.219 gap stands for the sessions this row covers | Field evidence (harness 2.1.219): six lanes, zero handbacks by morning despite near-complete work — builders' review seats woke the advisor instead of the builder, all six stalled for hours on finished soaks until an advisor sweep resumed each lane; plus the induction trap (re-wake proven early in-session only proves the active-task regime); plus **3 watchdog design failures and 3 false alarms** from mtime and process-table signals that rule out those cheaper substitutes. **Background-session trial (2026-09-15, §5):** the re-check trigger fired; the re-check ran; the outcome is **keep** — see §5.5 (receipt not clean; nothing retires on this receipt) | The harness delivers completion notifications reliably to the **spawning agent** for cross-session headless launches — not merely to an in-process team lead — → re-walk this row and consider retiring the heartbeat; OR past re-walk (#903): **agent teams** were walked and found not to be the nearest primitive (experimental, in-process only, cannot outlive the lead, absent entirely under `-p`); OR field evidence that the sweep's own classes produced a false `terminal` |
| **Dispatch-outcome chokepoint (`lib/dispatch_outcome.py`, outcome minting in `lib/engine_dispatch.py`)** — the shared vocabulary and terminal-outcome chokepoint for review and write dispatches. **Retired (issue #1269):** the forfeit ledger (`lib/forfeit_ledger.py`) and its attribution decider — a durable append-only record with no remaining reader after the refusal-rate measurement was cut (~1,200 lines of machinery, lock, and dedupe path removed; preflight refusals still return to the caller). Named consumer: the runner's own outcome minting | The nearest primitives today are the host's own task/agent telemetry and `engine_dispatch`'s supervisor journal — neither keeps a cross-run, cross-session, attributed record a later session can read | Field evidence (#747) motivated the ledger; with no reader the stack was retired while the outcome chokepoint was preserved | A platform primitive that carries durable, attributed, cross-session dispatch-outcome telemetry the advisor can read → revisit; **or** sustained field evidence that the supervisor journal plus vet discipline suffice → keep the chokepoint only |
...
| **Per-slot target boundary, policy home, and provisioning authorization (`lib/pilot_boundary.py`, `lib/pilot_policy.py`, `lib/pilot_provision.py`), issue #825** — exact-origin bindings, protected-target refusal, policy document resolution outside reach, results-only traveling verdicts, and the `authorize_credentials` chokepoint gating every credential-producing call. Named consumers: sub-issues C7 (#829) and C8 (#830) — C7 landed as the live in-repo consumer of `gate_provisioning`, `gate_datastore_identity`, and `require_declarations_exercised`; **C8 (#830) has now landed** and closes the remainder: `lib/launcher.py`'s `launch_build` stamps slot, generation, and boundary into the launch ledger, and `lib/launch_ledger.py` folds and projects them (`boundary_record`, the `slots` block in `count`) using `pilot_boundary` and `pilot_provision` — the in-repo half of the traveling-verdict path. The provisioning-time `authorize_credentials` call remains **closed by the C8 charter duty** (the advisor's slot-provisioning duty) — a human-driven advisor call, not an in-repo orchestrator import | None today — no host primitive enforces an application-level target allowlist or holds a policy document outside a branch's reach | The ratified design (#660 §3, §10) places the boundary in the framework because the check must run before any credential exists and must not route through branch-controlled code | Row re-walked at C8 (#830) landing and §13 residue closed; **or** the surface is retired; **or** a platform primitive appears and absorbs it |
| **Pilot slot lifecycle + provisioning journal (`lib/pilot_lifecycle.py`, `lib/pilot_journal.py`), issue #823** — the slot state machine, serialized generation allocation, the durable provisioning journal, and the partial-failure report. Named consumers are the epic's successors — **C7** (broker-side stale-generation enforcement, design seam S1), **A2b** (reclaim, which consumes the state machine), and **C8** (the owner-facing partial-failure report). C7 landed — it consumes `generation_check` at broker admission and `pilot_journal.effect()` / `mark_applied` for the server receipts — and **A2b landed too**: `pilot_reclaim` consumes `pilot_lifecycle` and `pilot_journal.replay`. **C8 (#830) has now landed** and closes the remainder: segment-aware replay (`pilot_journal.replay_sources`, `pilot_reclaim.aggregate_replay`) so a rotated slot's journal stays readable across retained segments, and the partial-failure gate (`pilot_journal.partial_failure_report`) — **closed by the C8 charter duty** (the advisor's partial-failure-report duty), a human-driven advisor call, not an in-repo orchestrator import; disclosed rather than claimed compliant because no non-test module imports `partial_failure_report` | No host primitive carries a per-slot lifecycle record with generation fencing identity, a crash-honest before-and-after provisioning journal, or a fail-closed partial-failure gate; the nearest in-repo things are `lib/file_lock.py` (an engine-apply lock with TTL-based stale reclaim, which is the wrong shape here because TTL reclaim is exactly the "second read of the same liveness marker" this design refuses) and `lib/heartbeat.py` (liveness, not allocation) | The design's own requirement that a journal written only on success reports a shared effect as never having happened; plus the repo-carried incident receipts named on issue #823 — the LEDGERS §1.1 launcher row's reconstructed-prompt shared-checkout collision, and the worktree-guard row's 8 in-repo plus 5 sibling uncommitted-work wipes | Row re-walked at C8 (#830) landing and §13 residue closed; **or** the unconsumed surface is retired rather than left to bit-rot; **or** a platform primitive appears that carries durable per-slot allocation with generation fencing and absorbs it |
| **Per-slot browser topology, context-side seed injection, and live provisioning gate (`lib/pilot_browser.py`, `lib/pilot_context.py`, the new `lib/pilot_provision.py` gate surface), issue #829** — each slot gets its own browser and automation server with generation fencing at broker admission; context creation enforces capture-option matching and verify-at-seed; the live declare-and-exercise and datastore-identity strength gates run at provisioning time. Named consumers: sub-issues C8 (#830), C9, and B5. **C8 (#830) has landed without wiring this surface** — no non-test module outside the `pilot_*` graph imports `pilot_browser` or `pilot_context` for orchestration, so per-slot browser topology, context-side seed injection, and the live provisioning gate still have no wave-orchestration caller. **Open §13 residue:** **C9**, **B5**, and the C8 orchestration wiring that was expected but is absent | None today — no host primitive provisions a per-slot automation server with generation fencing, nor enforces capture-option matching at context creation | The ratified #660 §9 topology ruling and §10's refusals, and the S1/S3 seams — the framework must own per-slot browser isolation and context-side injection because branch-controlled code cannot be trusted to enforce them | C9 and B5 land and the residue closes, at which point the row is re-walked; **or** they do not and the surface is retired rather than left to bit-rot; **or** a platform primitive appears and absorbs it |
| **Pilot reclaim safety + reassignment fence (`lib/pilot_reclaim.py`, `lib/pilot_fence.py`), issue #824** — quarantine-never-delete with a crash-honest sidecar, a sweep that warns rather than reaps, deletion gated on a bound terminal receipt plus an elapsed 72-hour grace, journal rotation into retained segments, and the reassignment acceptance probe's grading and fail-closed retirement. Named consumers are the epic's successors — **C7** (broker-side fencing consumes the reassignment verdict), **B5** (stand-up/teardown invokes the sweep on acting runs), and **C8** (the owner-facing report reads sweep warnings). **C7 has landed** on broker fencing. **C8 (#830) has landed** journal-segment and `aggregate_replay` machinery but **not** the sweep-warning reader — no non-test module calls `pilot_reclaim.sweep` or folds sweep warnings into an owner report. **Open §13 residue:** **B5**'s sweep-on-acting-run invoker and **C8**'s sweep-warning reader (closed by the C8 charter duty once wired, but no in-repo caller today); disclosed rather than claimed compliant under CONVENTIONS §13 | No host primitive carries a quarantine store whose deletion predicate refuses a second read of the liveness marker that declared the occupant stale; the nearest in-repo thing is `lib/file_lock.py`, whose TTL-based stale reclaim is exactly that refused shape, and the nearest external thing is the reference project's `scripts/worktree-quarantine.js`, which is git-worktree-specific and does not generalize. | The design's own settled parameters — 72-hour grace, sweep on the next acting run, past-threshold warns, disk pressure never authorizes deletion — plus the LEDGERS heartbeat row's **3 watchdog design failures and 3 false alarms** from mtime and process-table signals, which are why those two are liveness sources that can never be terminal. | B5 lands and C8's sweep-warning reader is wired (or closed by the C8 charter duty at orchestration), at which point the row is re-walked; **or** those consumers do not land and the unconsumed surface is retired rather than left to bit-rot; **or** a platform primitive appears that carries a crash-honest quarantine store with an independently-observed-terminus deletion gate and absorbs it |
| **Cleanup effect receipt, containment resolution, and resurrection planner (`lib/pilot_cleanup.py`, plus the policy's `datastore.containment` declaration), issue #831** — exercises a project's declared cleanup command against planted sentinels, records a pass/fail containment receipt bound to policy and source state, resolves how containment is assured (`permissions` / `single-slot` / `receipt` / `refused`), and plans resurrection without executing it. **C10 (#832) landed on `main` as PR #923** (not yet in this stack's base): the conformance run's `cleanup-end-to-end` exercise passed there — plant → cleanup → `cleanup_effect_receipt` → `registry_record` → `receipt_valid_for` → `resolve_containment` (mode `receipt`) → `resurrection_plan` produced, never executed — and `lib/pilot_conformance_cleanup.py` on `main` is the first non-test caller. **Open §13 residue:** C7 (#829) and C8 (#830) still landed without orchestration wiring; no module in this tree outside the `pilot_conformance_*` graph imports `pilot_cleanup` for wave-orchestration or provisioning, and C8 as ratified still does **not** execute the resurrection plan or take the receipt at provisioning. #859 (landed) reshaped the receipt's source binding first. Disclosed rather than claimed compliant | None today — no host primitive proves that a project's own destructive command is contained to a namespace, nor binds such a proof to the source state of the script it tested. The nearest in-repo things are `lib/pilot_boundary.py`'s observer execution (identity observation, not effect containment) and `lib/pilot_journal.py` (recording effects, not bounding them) | The owner-ratified design (#660 §7, decisions D2/D9/D12) makes the receipt load-bearing rather than defensive — resurrection is a **destructive** operation on the slot's namespace, so an overreaching cleanup would damage siblings on **every relaunch**, not only at teardown | B5 lands and an acting-run orchestrator imports `pilot_cleanup` at provisioning (or the C8 charter duty wires it at orchestration), at which point the row is re-walked; **or** the unconsumed orchestration surface is retired rather than left to bit-rot; **or** a platform primitive appears that proves destructive-command containment and absorbs it |
| **Pilot auth exercises (`lib/pilot_identity.py`, `lib/pilot_horizon.py`, `lib/pilot_mint.py`), issue #828** — the D3 identity-probe exercise and mid-wave lapse episode, the launch-time credential validity margin (`deadline + margin ≤ horizon`), and minted sign-in exercises (flag-scope check, gate-off runner, live sentinel probe with positive control). Named consumers are the epic's successors — **B5** (#827 for wiring `wave_margin` into launch preflight and runtime terminus), **C7** (#829 for browser-context identity probes), and the preflight layer for live identity and sentinel exercises — so like #822, #825, and #823 this ships as pinned mechanism **ahead of its first caller, disclosed rather than claimed compliant** under CONVENTIONS §13. This build **closes the rest of #822's remaining A1 residue** by becoming the live consumer of `pilot_contract`'s `supports_unattended_horizon`, `is_exercised`, and `require_exercised`, `pilot_slot`'s `account_keys`, and `pilot_probe`'s `classify` (via `lapse_step`); #823 had already closed `validate_slot_id`, `validate_generation`, `slot_account_set`, `format_slot_ref`, and `parse_slot_ref`. **`pilot_probe`'s `routes_to_lapse` and `is_infrastructure` remain unconsumed** — only `classify` is called from B6 | No host primitive exercises whether an identity probe discriminates, a mint gate refuses, or a credential outlives the wave; the nearest in-repo thing is `pilot_contract`'s declare-and-exercise registry (`is_exercised` / `require_exercised`), which records that an exercise happened but does not run the live probe, gate-off test, or margin comparison | The ratified design (#660 §4, §5, §10 and decisions D3/D10/D11) places these exercises in the framework because a project must not be able to *assert* that its identity probe discriminates, its mint gate refuses, or its credential outlives the wave — each is exercised and recorded | The epic's consumers land (B5/C7/preflight) and the residue closes; **or** they do not and the surface is retired rather than left to bit-rot; **or** a platform primitive appears and absorbs it |
| **Per-slot app lifecycle + wave deadline runtime (`lib/pilot_appctl.py`, `lib/pilot_wave.py`, plus substrate amendments in `lib/pilot_contract.py`, `lib/pilot_provision.py`, and `lib/pilot_journal.py`), issue #827** — per-slot `devCommand` stand-up with wave-wide endpoint fencing, readiness attribution, durable instance records, launch-anchored monotonic deadline, durable park latch, and two-phase wave-end teardown with injected step handlers. Named consumers are **C8** (#830, wave orchestration) and **B4**'s live `app-lifecycle` exercise (#826). **C8 (#830) has landed without wiring this surface** — no non-test module imports `pilot_wave` or `pilot_appctl` for orchestration. **Open §13 residue:** **C8**'s wave-orchestration caller (closed by the C8 charter duty once wired, but no in-repo caller today) and **B4**'s live `app-lifecycle` exercise (#826); disclosed rather than claimed compliant under CONVENTIONS §13 | None today — no host primitive carries per-slot process lifecycle with deadline enforcement; the nearest in-repo things are `lib/engine_dispatch.py` (process supervision, but engine-CLI shaped and not per-slot) and `lib/heartbeat.py` (liveness, not process control or deadline enforcement) | The ratified design's own requirements: a bind conflict is evidence of a failed fence, work is preserved at the terminus, and an absent handler is not a skipped step. This ships ahead of its live consumers — **C8** (#830) and **B4**'s live exercise (#826) — disclosed rather than claimed compliant, exactly as the #822, #825, and #823 rows do | B4 (#826) lands and C8's wave-orchestration caller is wired (or closed by the C8 charter duty at orchestration), at which point the row is re-walked; **or** those consumers do not land and the unconsumed surface is retired rather than left to bit-rot; **or** a platform primitive appears that carries per-slot process lifecycle with deadline enforcement and absorbs it |
| **Launch ledger slot/generation/boundary grammar and segment-aware journal replay (`lib/launch_ledger.py`, `lib/pilot_journal.py`, `lib/pilot_reclaim.py`), issue #830 (C8)** — stamps which durable slot and generation each launch reserved, folds results-only boundary blocks into the batch `count` report's `slots` block, and replays a rotated provisioning journal across retained segments (`replay_sources`, `aggregate_replay`). Named consumer: the advisor's slot-provisioning duty (C8's charter) reads the ledger's slot and boundary fields and the batch report's `slots` block; **`lib/launcher.py`'s `launch_build` is the live in-repo caller** that populates slot, generation, and boundary on reserve. **D11a (#833) landed on `main` as PR #925** (not yet in this stack's base — acceptance matrix), but the conformance report's passing exercises on `main` (`artifact-store`, `cleanup-end-to-end`, `mint-gate-off`, `reclaim-sweep`, `wave-headless`) do not name `launch_ledger`, `replay_sources`, or `aggregate_replay`; segment-aware replay still has no non-test orchestrator caller outside the `pilot_journal` / `pilot_reclaim` internals in this tree | None today — no host primitive records which durable slot and generation a launched session occupied, nor folds a rotated append-only journal across its retained segments | The ratified design requires a wave's batch report to answer "which slot failed" and a rotated slot's history to stay readable across journal segments — neither rides on the host's subagent/Task spawn or `engine_dispatch`'s supervisor journal | The conformance run's closed `REQUIRED_SURFACES` inventory gains and exercises launch-ledger stamp and segment-replay surfaces, at which point the row is re-walked; **or** the advisor's slot-provisioning duty wires `replay_sources` / `aggregate_replay` at orchestration; **or** the unconsumed segment-replay surface is retired rather than left to bit-rot; **or** a platform primitive appears that carries durable per-slot launch accounting and segment-folding journal replay |
| **Review-receipt handback refusal class (spike #624)** — deterministic refusal of `gh pr ready` / non-draft `gh pr create` in a mechanically-marked full-lane worktree lacking an allowlisted review receipt. **Shipped dark in 0.25.0** — built, in-tree, and **not wired**; enforces nothing today; its stated carrier — the #482 owner-authority hook — retired in issue #1264 (C5 merge-gate retirement); **arming is owned by #954** (retrospective would-refuse audit → shadow mode → preconditions → owner decision), which must re-establish a carrier before this class can arm | Plugin-shippable native permission rules — same platform gap as the retired owner-authority gate (#482): the platform's permission model does not today let a plugin ship declarative deny/ask permission sets (it must be a hook) | **Licensing evidence (spike #624 corpus):** hand-composition as the norm (11 reconstructions), five silent roster/vendor drifts including the compliant run, the #622 record loss, and adjudication F6's finding that prose enforcement contradicts the corpus's own text-doesn't-buy-behavior evidence — plus the owner's §4 refuse-and-park ruling. Owner-ratified 2026-07-30; honesty-gate discharge under owner ruling 2026-08-09 (the §2 "No new honesty/grounding gates…" entry is discharged for this gate only on that corpus as the qualifying escape) | Plugin-shippable native permission rules land → move the gate onto the primitive and retire the hook; **or** generic light/micro lane receipts subsume the class; **or** the two-release false-refusal review (see the §2 review-receipt handback refusal entry) finds a confirmed false refusal after #954 arms the class → reopens the entry |
| **Charter-aware compaction context (`lib/charter_detect.py`, `hooks/compaction_context.py`, SessionStart recovery block)** — detects showrunner/workhorse charter from the session transcript and injects a per-charter preserve/drop skeleton at PreCompact via a top-level `additionalContext` key (not `hookSpecificOutput`); SessionStart re-injects charter recovery after compaction. Named consumer: the `checkpoint` skill and both charter sessions (showrunner, workhorse), which re-read their charter after compaction because of it | Native, charter-aware context preservation across compaction — a harness primitive that lets a plugin declare durable session context that survives summarization | Compaction silently drops charter fidelity, and auto-compact does it unattended — exactly when these long-lived charter sessions are working; the harness accepts only a top-level `additionalContext` on PreCompact (probe-verified Claude Code 2.1.222), not the `hookSpecificOutput` wrapper other hooks use | The harness gains declarative persistent session context, **or** PreCompact gains a supported `hookSpecificOutput` contract that makes the top-level-key dependency moot → move onto the primitive and retire the bespoke hook |
| **Sibling worktree delta observation (`lib/sibling_worktree_probe.py`, `engine_dispatch.py` fold chokepoint), issue #754** — at write-run open and again at terminal fold, snapshots every other registered worktree (HEAD sha, porcelain sha256, HEAD reflog entry count) and emits an unattributed observed delta on the folded result. Named consumer: the Workhorse orchestrator's post-dispatch observation duty (owner ruling 2026-07-30) | The retired post-dispatch confinement tripwire (#355) detected primary-checkout excursions only; `git worktree list` plus host subagent spawning is the nearest platform primitive, but neither compares sibling worktrees across concurrent authorized dispatches nor folds an advisory delta into the write result without gating | Owner ruling 2026-07-30 after PR #710 round 4 caught an implementer resetting a build worktree that was not its own — observation by default, advisory not a gate. Licensing evidence: that catch plus the ruling. The probe is bounded (aggregate deadline, sibling cap, `GIT_OPTIONAL_LOCKS=0`) so advisory observation cannot become a terminal `git-preflight-timeout` on its own | A platform primitive observes cross-worktree mutations with durable attribution the orchestrator can trust → retire the bespoke probe; OR sustained field evidence that the unattributed deltas are pure noise with zero review value across measured multi-dispatch waves → revisit whether the divergence still earns its keep |
| **Ledger-driven wave watcher (`lib/wave_watch.py`, issue #982)** — a ledger-driven watcher over one launch batch with a `loop` verb that re-arms internally, passing over benign wakes (PR-set and stack-state changes, reported at exit), until a lane-ending event, a refusal, or its ceiling, and that refuses to start beside a live loop on the same batch; prints one JSON line, and exits (exit 0 on an event, exit 1 on a refusal), armed as one harness background task per batch; folds heartbeat ending states, pid liveness plus transcript quiet window, and PR-set deltas under stated precedence. Still-open limits (PR #988): PR changes in the gap between separate invocations (bare `run` is a one-shot and never reports them) are not reported — closed within a `loop` arm-chain, where the PR baseline threads across internal arms; a mistyped batch id is indistinguishable from a quiet batch; a started lane that never stamped still gets the liveness check and surfaces `lane-never-stamped` at a timer while `builder-exited` still surfaces it on pid death. Named consumer: the Showrunner charter's orchestration duty 9 wave-watch arm-pointer — `skills/showrunner/SKILL.md` duty 9, with the arming pattern one hop away at `skills/showrunner/reference/wave-watch.md` | The host's own background-task completion notification / wake delivery and its task-status surfaces — on harness 2.1.219 the primitive exists but, with the spawning agent dormant, completions reach the **root session**, not the spawning builder (same as the adjacent heartbeat row), and no host primitive folds a **launch-batch ledger** plus per-lane heartbeat and PR-set state into a single first-qualifying-event block with a stated precedence | Field evidence (PR #988): hand-rolled per-session watch loops failed quietly and repeatedly — a **double-backgrounded loop that orphaned**, a **hand-typed PID list that went stale** the moment an unpark launched a new builder, and a **10-hour dead-watcher hole**; each failure looked like a calm wave. The design property that makes the divergence worth keeping: `loop` re-arms inside the tool with no daemon to orphan — the failure mode of every hand-rolled substitute | The harness delivers batch-level completion/wake signals reliably to the **spawning** session so a watcher is unnecessary → re-walk this row and retire the tool; **or** the heartbeat sweep's own accounting shows the watcher's events never arrived earlier than the scheduled sweep across measured waves → revisit whether the divergence earns its keep |
| **Issue-contract build-ready check (`lib/issue_contract.py`, issue #932)** — subcommand `check-build-ready --body-file` reads an issue body and reports whether the `Anchor (<kind>):` slot header is missing, declares no kind, declares an unknown kind, declares more than one kind, or is empty (JSON result; anchor **shape** only — whether the anchor actually resolves is a separate check owned by a later work item). Named consumer: the Showrunner charter's build-ready marking step (`skills/showrunner/SKILL.md` duty 3) plus `skills/showrunner/reference/issue-contract.md` — both shipped in the **same** PR as this validator | None today — no platform primitive validates an existing issue body against a project-defined doctrine contract at marking time; GitHub issue templates and issue forms can *shape* a new issue at creation but cannot check an existing body when the advisor marks it build-ready, and do not apply to issues filed before the template existed | §13 requires a named consumer plus this ledger row for any new deterministic validator; this is **advisory** — it always exits 0 and enforces nothing on its own; the Showrunner charter's build-ready duty is what declines the marking. It is **not a mechanical enforcement boundary and is not claimed as one**, so it is **not a new member of §2's plugin-owned-enforcement-machinery class** and claims no part of the #624 discharge. Owner-ratified precondition: spec `front-half-sdlc-core-6181ee` FR-8 and FR-1's acceptance rule (approved 2026-08-07), epic child C1 (#932), and the owner's 2026-08-16 declared-kind ruling (anchor kind declared in the slot header, never inferred from citation prose) | A platform primitive that can validate an existing issue body against a project-defined contract at marking time lands, OR the issue contract itself is retired |
| **Vet-checks key validator (`lib/core_md.py`: `validate_vet_checks`, `read_vet_checks`, `write_vet_checks`, CLI `vet-checks`/`write-vet-checks`)** — validates the optional `vetChecks` key of core.md's JSON block, the read verb, and writer refusal on malformed input. Named consumer: the showrunner vet (`skills/showrunner/reference/vet-receipt.md` § Project vet checks) and `configure`'s writer and view — shipped in the **same** PR | None today — no host primitive validates a plugin-owned calibration key | Without it a malformed key reads as "no checks" and the vet silently skips vet-carried rules; it enforces nothing mechanically (no hook, no CI gate, no PreToolUse boundary), so it is **not** a member of §2's plugin-owned-enforcement class. Owner-ratified precondition: the verification-strategy adoption rulings (2026-08-27/28, receipts on #1105 and weekly-eats-hq/weekly-eats#1109) and the owner's 2026-09-25 ruling that the checks live as a key in the JSON block (issue #1238) | A host primitive for project-calibration schema validation lands, OR the vet's vet-checks triggered field retires |
| **Owner-half writer (`lib/vet_slot.py`, verbs `write`/`check`)** — writes the `## Advisor vet` slot only between the `advisor-vet` and `build-record` markers, after checking the body read and that the build record's followups marker and the receipt's dispositions marker list the same ids; `check` runs the same comparison with no write. Named consumer: the showrunner vet (`skills/showrunner/reference/vet-receipt.md` § The owner-half write — register, and duty 4's merged-PR backstop) — shipped in the **same** PR | None — no host primitive checks a PR body against a comment | Without it a follow-up drops silently at vet; it refuses only its own write (no hook, no CI gate, no PreToolUse boundary), so it is **not** a member of §2's plugin-owned-enforcement class. Owner-ratified precondition: the owner's 2026-09-25 ruling on sitting item 30 ("30 a"), graded as machinery with executed field evidence (14 of 301 follow-ups dropped across 6 of 41 PRs in one consuming project's week) | A host primitive for structured PR metadata lands, OR a week of vets shows zero writer refusals and zero dropped follow-ups |
| **Register-quote exact-text check (`lib/register_check.py`, issue #940)** — subcommand `check --register --body-file --child` compares each quoted register block in a child issue body byte-for-byte against the named register file and requires every entry whose `*Consumers:*` line names the child to be quoted in that body (both directions). Named consumer: the three charter invocation lines shipped in the same PR — showrunner duty 2 filing, showrunner duty 3 package-read verification, workhorse §1 build intake — plus `skills/showrunner/reference/register-check.md` | None today — no platform primitive compares a quoted block in an issue body byte-for-byte against a file in the repository; GitHub issue templates and forms shape a *new* issue at creation and validate nothing about an existing body, and no code-owner or branch-protection rule reaches issue text at all | §13's named consumer plus this ledger row; this one **is** a decider whose result the charters act on (a filing is fixed, a build **parks**), so unlike C1's advisory check it uses exit codes — and it enforces nothing mechanically: no hook, no CI gate, no PreToolUse boundary, so it is **not** a new member of §2's plugin-owned-enforcement class and claims no part of the #624 discharge. Owner-ratified precondition: spec `front-half-sdlc-core-6181ee` FR-31 and register entry R3 (approved 2026-08-07, R3 as amended 2026-08-08), epic child C9 (#940) | A platform primitive that can compare issue-body text against a repository file at filing time lands, **or** the epic-contract machinery (FR-28/FR-31) is retired |
| **Package-read audit trail receipt (`lib/package_read_audit.py`)** — the epic package read's audit trail is machine-written and machine-checked: writer verbs open an invocation and record each round and the verification pass, and `check` audits a trail for completeness and well-formedness against the element list in `skills/showrunner/reference/decomposition.md` § The audit trail. Validation is a declaration per record kind read by both the writers and `check`, so a record the writers would refuse cannot read as conforming. Named consumer: the showrunner charter's decomposition duty (duty 3) and that reference's package-read section | None today — no platform primitive records or validates a project-defined review-round audit contract in a repository document | CONVENTIONS §13's named consumer plus this ledger row. It **is** a decider whose result the charter acts on — a `nonconforming` exit is what holds a package from being treated as verified — but a completeness-and-shape verdict over advisor-asserted records only, never a re-decision of convergence or the park call; and it still enforces nothing mechanically (no hook, no CI gate, no PreToolUse boundary), so it is **not** a new member of §2's plugin-owned-enforcement class and claims no part of the #624 discharge. Owner-ratified precondition: spec `front-half-sdlc-core-6181ee` FR-32 and UFR-7 (approved 2026-08-07), epic child C6 (#937), and the owner's 2026-08-15 field-evidence direction that the package read's per-round receipt be written as an artifact rather than left in a session log, and the 2026-08-18 ruling re-scoping the validator | A platform primitive that can validate a project-defined audit contract in a repository document lands, **or** the epic-contract machinery (FR-27/FR-32) is retired |
| **CommonMark fence recognition (`lib/md_fence.py`, issue #940)** — shared stdlib-only line classifier for CommonMark §4.5 fenced-code-block **recognition** (opener, content, closer, inert mask; indent 0–3); no I/O. Named consumers: `lib/register_check.py` and `lib/issue_contract.py` — both shipped in the **same** PR as this module, which is what discharges CONVENTIONS §13 for those two validators. **Known residue:** `lib/guardian_lens_docs.py` still carries its own `_FENCE` opener regex and a naive `in_fence = not in_fence` toggle — same defect class, no marker/run-length/indent/trailing-text rules — for docs reference extraction (tags path tokens while **keeping** fenced blocks, not masking live quotes) | None available without taking a CommonMark parser dependency — the owner declined that dependency | §13 requires a named consumer plus this ledger row for any new shared module; it enforces nothing mechanically: no hook, no CI gate, no PreToolUse boundary, so it is **not** a new member of §2's plugin-owned-enforcement class and claims no part of the #624 discharge. Owner-ratified precondition: owner ruling **10 - d** (2026-08-16) — rework the seam, not the patch | A CommonMark parser dependency is taken, **or** both consumers stop scanning markdown |
...
| **Owner-authority gate (minimal PreToolUse Bash hook, issue #482)** | Emitted a native `ask` on the enumerated merge/release/publish/force-push/push-to-default set, scoped to superheroes-calibrated projects; the never-merge floor as a mechanical tripwire behind the covenant's hardest line; supported an owner-calibrated allowlist (`owner-authority-allow.json`) that could pre-authorize an exactly-named workflow dispatch, with merge, release, and force-push structurally excluded (see `reference/owner-authority-allowlist.md`) | Owner ruling issue #1264 (C5 merge-gate retirement): merge approval rests on the owner's scoped word plus advisor discipline, not a mechanical per-merge checkpoint; the gate's false-positive cost (including a read-only census `grep` classified as `run-workflow` during the retirement build because its pattern list contained those words) outweighed its tripwire value once PHILOSOPHY promise 1 and the showrunner charter's merge duty carried the merge act without a mechanical floor | Issue #1264 (C5) |
| **Dogfood plugin-vs-repository semantics skew detector (`lib/version_skew.py`, issue #677)** | Detection-only check that compared watched semantics files (`lib/model_registry.py`, `lib/seat_map.py`, `lib/version_skew.py`) between the running plugin cache and the superheroes source repository; the seat-map compose receipt recorded `pluginVersionSkew` and could emit a named `plugin-version-skew` degradation on `checked-degraded` (`semantics-divergent` or `evidence-unreadable`) | Retired under issue #1264 (C5 merge-gate retirement) with the keep-or-retire K2 disposition; no host surfaces cache-vs-source skew today and the detector had zero recorded firings. The K2 rebuild trigger survives the retirement as a revisit-registry row on the standing collector — a real skew incident can reopen a rebuild proposal at a gardening pass | Issue #1264 (C5) |
...
- **No sixth review LENS.** The ban always meant *lens*: no sixth **risk-domain review
  ...
  **#131 (the benchmark)**. The owner-ratified (#474, 2026-07-20) **grounding seat** is
  ...
  dispatch condition is met and discharged by #609 stage 2** (the seat map itself shipped
  under #510), while the spec leg's Grounding seat is live as of **#515/#517**.
...
- **No new honesty/grounding gates without a named escape that penetrated every
  ...
  Discharged for the review-receipt handback refusal gate only — owner ruling
  2026-08-09; see the review-receipt handback refusal entry below.)*
- **No plugin-owned enforcement machinery.** *(Owner-ratified 2026-07-20, spike #475.)*
  ...
  option.   *(Scoped exception, owner-ratified 2026-07-31; amended issue #1264 (C5 merge-gate
  retirement): a minimal safety floor on an **irreversible local destructive action** —
  the worktree guard (issue #682) is now the sole member of this class recorded in §1.1;
  the owner-authority gate (#482) retired with issue #1264. A second member of this class
  requires **owner ratification per member, with recorded occurrence evidence** (same bar
  as the worktree guard). The banned class is
  **analysis and architecture enforcement** that belongs in the owner's toolchain — checks
  ...
  only*, not a general licence. See also the review-receipt handback refusal scoped
  exception below.)*
- **Review-receipt handback refusal (owner-ratified via spike #624, 2026-07-30).**
  *(Scoped exception.)* Deterministic refusal of `gh pr ready` / non-draft `gh pr create`
  in a mechanically-marked full-lane worktree lacking an allowlisted review receipt.
  **Shipped dark in 0.25.0** — built, in-tree, and **not wired**; enforces nothing today;
  **arming is owned by #954** (retrospective would-refuse audit → shadow mode →
  preconditions → owner decision). The licensing evidence is spike #624's corpus:
  hand-composition as the norm (11 reconstructions), five silent roster/vendor drifts
  including the compliant run, the #622 record loss, and adjudication F6's finding that
  prose enforcement contradicts the corpus's own text-doesn't-buy-behavior evidence. The
  §2 entry *"No new honesty/grounding gates without a named escape that penetrated every
  existing layer"* is discharged **for this gate only** on that corpus as the qualifying
  escape (owner ruling 2026-08-09). **Re-check trigger (arming, not dark ship):** when
  #954 arms the class, at each of the next two releases after arming the advisor reviews
  full-lane refusal occurrences from receipts/journals; **one confirmed false refusal**
  — a refusal against a build holding a genuinely valid certified receipt — is a defect
  that **reopens the entry**. **Honest scope:** this class is a **Claude-host, Bash-tool,
  honest-agent tripwire**; it does not cover aliases, wrapper scripts, non-Bash tools, or
  other hosts; **it is not a security boundary and is not claimed as one.** *(Cross-reference:
  the "No plugin-owned enforcement machinery" entry above; the "No new honesty/grounding
  gates…" entry discharged for this gate only.)*
- **No reviewer fallback chain** (owner-ratified 2026-07-31, rider 33). A **codex → cursor →
  disclosed-Claude reviewer** chain, proposed from a consuming project's field dataset, was
  declined for three reasons: (a) forfeit causes are **presumed self-inflicted until
  attributed**, and the attribution work is open; (b) **fallback chains normalize their own
  tail** — every hop needs a probe, a disclosure and accounting, or hop 2 quietly becomes the
  default; (c) a separate re-assessment of an alternative reviewer engine is still open.
  **Revisit (all three, conjunctive):** the attribution work lands **AND** the post-fix codex
  review-forfeit rate stays high **AND** the alternative-reviewer re-assessment closes
  favourably. Cite this ledger instead of re-proposing.
...
| **The `-f` flag on cursor write dispatches** (owner-ratified 2026-07-31). Worktree confinement for a `--trust -f` cursor engine subprocess remains **detection, not containment** — the same bound the worktree-confinement row above states. | A confined cursor write dispatch runs `cursor-agent` with `--trust -f`; the post-dispatch confinement tripwire is the detection layer, not a sandbox around the subprocess. | `-f` is retained deliberately because it also **immunizes the write dispatch against the machine-global mutable `~/.cursor/cli-config.json`** — one permission file shared by every cursor invocation on the machine, with concurrent writers observed in the field. A no-`-f` invocation inherits whatever the last writer left; a `-f` invocation does not. | Cursor ships per-invocation config isolation so a write dispatch no longer inherits a machine-global mutable permission file; OR Claude Code ships a native process-sandboxing primitive that can confine a spawned CLI (walk with the worktree-confinement row); OR a field incident where inherited global cursor config caused a write-dispatch outcome that review should have caught |
...
| **CLOSED — issue #1264 (C5 merge-gate retirement): the owner-authority gate is retired** (issue #482). The gate was a regex heuristic over command text, not a sandbox; it covered the enumerated merge/release/publish/force-push/push-to-default shapes but by construction did not catch every variant, was wired Claude-host only, and on process failure its fail-closed wrapper denied all Bash wherever the plugin was enabled. | **Moot.** Merge approval now rests on the owner's scoped word plus advisor discipline (`PHILOSOPHY.md` promise 1, showrunner charter merge duty); there is no mechanical merge floor. The enumerated risks this row tracked are governed by that doctrine, not a hook. | Owner ruling issue #1264 (C5): the gate's tripwire value was superseded by the reset merge act and its false-positive cost (including a read-only census `grep` refused as `run-workflow` during the retirement build) outweighed what remained. Record kept; residual closed. | N/A — gate retired |
...
| **Native in-session seats are declared live and never probed** (2026-09-16). Unlike `codex` and `cursor` external-engine dispatches, native in-session seats — the review lenses, the grounding seat, the check-runner, synthesis, the pilot — are **declared** live rather than exercised before use. | Any certification that rests on an unprobed native family rests on a **declaration, not a receipt**; such a certification owes a disclosure line saying so. CONVENTIONS carries the population rule (`native in-session seats are declared live and never probed`); this row records the consequence. A review receipt now names every review seat with no runner execution record — native in-session, fallen open to the host, or hand-landed — as an unprobed native seat in its disclosures, read from the transport each recorded seat event carries (2026-09-21). | Owner-accepted 2026-09-16 — probing every native seat on every dispatch would multiply cost and latency without a mechanical primitive to do it; `codex` and `cursor` are probed because their CLI contracts are external and variable. | A host primitive lets a plugin exercise native in-session seats before certification; OR a field incident where a native seat assumed live was demonstrably inert or misbehaving. |
...
| **Sibling worktree delta observation is detection, not containment — and it is unattributed** (2026-08-13, #754). After every terminal `dispatch-write` fold the runner compares sibling-worktree snapshots (HEAD sha, porcelain sha256, HEAD reflog entry count) and emits `siblingWorktrees` on the result. It DETECTS an observed delta in worktrees this run did not own; it does not prevent, refuse, or attribute. Disclosed blind spots, all accepted: concurrent authorized dispatches in different worktrees routinely produce legitimate deltas that cannot be attributed to this run; a write-then-restore inside the window moves none of the three signals; a `.git`-internal write is outside probed scope; a `git push` mutates a remote and leaves the local probe clean; a write to a **different** on-disk repo is outside scope; a detached child that writes after the post-snapshot escapes the sample; ignored-file churn `git status --porcelain` does not report; reflog expiry or gc can make a count decrease; a worktree added or removed mid-run may appear as appeared/disappeared without identifying when; and the aggregate deadline caps how many siblings can be probed before the observation goes `indeterminate`. | **Detection, not containment** — same posture as the retired #355 confinement tripwire row and the `check-runner` probe row: the security walls remain the owner merge gate, review discipline, and git recoverability. The delta is advisory evidence for the orchestrator's narration, never a gate on `ok`/`terminal`/`reason`. | Owner ruling 2026-07-30 — convert PR #710 round 4's lucky catch into observation by default without implying attribution concurrent authorized dispatches cannot support. | A platform primitive supplies durable cross-worktree mutation telemetry with attribution the orchestrator can trust → move the guarantee onto it and retire the bespoke probe; OR field evidence that the deltas are pure noise with zero review value across measured multi-dispatch waves → revisit whether the row still earns its keep. |
| **The dirt probe's status-vs-content structural limit** — `git status --porcelain` carries **status, not content**, so an attempt that rewrites a file already `??` or ` M` at baseline leaves the entry record byte-identical and reads **clean**. Reproduced on PR #1128; pre-existing and identical on `main` (the shipped digest hashed the same status text). | Confined to files already dirty at baseline; a rewrite of a clean-at-baseline file is still caught. Closing it needs **content identity** (hashing file contents), a different design from an advisory probe. | Owner-accepted 2026-08-24 via the authorized fix round on #1122; same **dispatch-layer reliability posture** as the dispatch-layer same-user process exposure row above (findings graded as reliability, by consequence and recoverability); PR #1128 reproduction. | A dispatch-integrity incident traced to a baseline-dirty file being rewritten, or any move of this probe from advisory to authoritative. |
...
**Ruled (historical, 2026-07):** the covenant line that read as if merge execution itself was
never delegable was repaired after four days of practice where delegated `gh pr merge` plus owner
gate clicks was the actual shape — the text did not say which reading was right. The never-delegable
act is **approval** — the gate click, the release cut, the publish decision. **Merge-command
execution** was delegable, but **only where a mechanical per-merge approval checkpoint existed on
that host or path**; where none existed, execution stayed in the owner's hands. **Release PRs and
anything needing a force-push were never delegated.** **Approval stayed per-PR** (owner ruling;
"approve once, execute five" not adopted). The owner-authority gate was a **backstop, not an
authorization boundary** — delegation stood on advisor discipline with the gate behind it, never the
reverse. Record kept; superseded below.

**Resolved 2026-07-29 (#706, historical):** the owner amended `PHILOSOPHY.md` promise 1 — the
constitution drew the approval/execution distinction itself: the decisions that bind the owner are
never made on their behalf, executing an approved merge could be delegated **only** behind a
mechanical checkpoint that guaranteed the owner's approval every time, and releases, publications, and
rewritten history stayed in the owner's hands. Record kept; superseded below.

**Superseded 2026-09-16 — issue #1264 (C5 merge-gate retirement):** the merge act now stands
without a mechanical merge floor. Approval is never delegated: it is a scoped word in chat after
the PRs are talked through, recorded by PR number beside the word and on each PR's owner half; the
advisor executes merges inside the scope after the lane's review and verification evidence, CI green
on the recorded head, and a current branch, and reports each merge at once in the thread that gave
the word; a PR opened after the word or changed materially asks again, judged against the
material-consequence line; a red merge train is fixed under the word when the fix is craft;
force-push proceeds on a word after the reason is stated; releases and publishing stay the owner's
click unless handed over in the moment. **There is no mechanical merge floor.** The owner-authority
gate (issue #482) is retired (§1.2); merge approval rests on the owner's scoped word plus advisor
discipline. The host-conditional per-merge-checkpoint clause above is **retired** with it.
**"Approve once, execute five" is superseded** — a scoped word covering several talked-through PRs is
exactly the shape once declined; the record states supersession, not silent adoption. Delegated
inside the word: issuing the merge command, sequencing, `update-branch`, CI-green waiting, conflict
resolution under an advisor-authored recipe, post-merge hygiene — with the lane's review and
verification evidence, CI green on the recorded head, and a branch current with its base as
preconditions that never waive. **Known divergence closed:** `PHILOSOPHY.md` promise 1 and the
covenant's promise 1 now agree — approval is the owner's scoped word; inside it the advisor executes
each merge. The #706-era divergence (execution never on the owner's behalf vs approval-only
never-delegable) and the checkpoint-bar disagreements recorded under #706 are **closed** by the
reset, not left disclosed.

**What it closes, and what it does not.** This entry closes R3's historical re-check triggers, the
#706 checkpoint-bar open questions, and the covenant/PHILOSOPHY execution-wording divergence they
tracked. **It makes no wider coherence claim** beyond the merge act.

`README.md`'s review-independence bullet reads "merge approval stays with the owner": that one is
**still true and unchanged** — the same document's Showrunner section carries the scoped-word
qualification and advisor execution inside it. The complete restatement lives in `PHILOSOPHY.md`
promise 1, the covenant, `CONVENTIONS.md`, and the `showrunner` charter's merge duty; the
`workhorse` charter deliberately does not repeat the standing orders, deferring to the covenant for
them.
...
## 5. Background-session trial receipt

The decide-by for the orchestration retirements. One wave was run by hand on the harness
primitives, across the three Claude accounts on this machine, with no replacement code written
...
Three lanes, one per account, each a real task with a verifiable artifact.

| Lane | Account | Session id | Launch acknowledgement | Idle notification | Terminal state | Result retrieved by |
|---|---|---|---|---|---|---|
| A | `~/.claude` | `323cc06c-2fb9-4037-8db3-597b00949c94` | `Starting background service… / backgrounded · 323cc06c` | subscribed; **delivered at this session's next turn boundary**, long after the lane went idle | `done`, later `blocked` | `result.json` = `{"lane":"A","sum":76127}` |
...
path is not evidence about notification. All five were eventually delivered, at this session's next
turn boundary rather than during the trial; see 5.2. Five subscriptions in total, every account represented:
...
Seven further sessions carried the rest, beyond the seven named above: a contract smoke
(`daf1d3b4`), a turn-end survivor (`22818a88`), a background primary-checkout probe (`0df35673`),
...
**A session dying at turn end — exercised.** A disposable `claude -p` controller under
...
seconds cold, and `state: blocked`; a second wedged lane, lane A (`323cc06c`), had a live pid, a transcript 144.0
...
**A lost idle notice — exercised. The notice was late rather than lost, and it is the finding that decides the set.** Five
...
was stopped outright, and three reached `done` within a minute of their subscription.
...
original text claimed no notice was delivered, which was wrong — the trial had confused *did not
arrive within the turn* with *did not arrive*. The distinction changes the reason every
notification-dependent line below gives, and it changes two delete-when conditions, which had been
written so loosely that the corrected evidence would have satisfied them.

The bound on this observation: five subscriptions across three accounts, three transition kinds
(idle, stopped, and done), one delivery event at one turn boundary. One flush event cannot separate
delivery that happens only at boundaries from delivery that was blocked by something else and flushed
at that boundary. A re-run that places a subscription and then idles across several boundaries would
separate them. It says nothing about an interactive supervisor.
...
The supervision view does not cross accounts, and wake and supervision are different channels with different reach.
...
Every new capability this trial found attaches to the **background** shape, and the launcher does
not spawn that shape.

A background session launched from a repository's primary checkout was given a task that
overwrites a tracked file. The harness created `.claude/worktrees/<name>` on its own branch,
locked it, ran the session inside it, and left the primary checkout untouched. The same repository,
same task, run as `claude -p`, edited the primary checkout directly and created no worktree at
all. A live `claude -p` session does appear in the session listing, but as an interactive session
...
- **Wave-watch arming and the re-arm ritual.** **Needed.** *Evidence:* five idle subscriptions
  ...
  **headless spawning** session **while that session is still working** — inside the turn, not at
  ...
  root session standing in for it.
...
every subscription and is **consistent with** what arrived — each of the five subscriptions produced
