...
| **"How many vets ago" is not reliably computable from the artifacts** (2026-07-29, #672/#694). The collector's escalation tripwire fires when an item was **proposed two or more vets ago**, and the charter tells the advisor to compute that age by counting vet receipts. Three successive attempts at the counting rule each fixed one boundary and exposed another (late by one; then an implicit anchor risking early by one; then an in-place-corrected receipt whose posting time predates intervening vets). | **The rule counts posted receipt artifacts, not vet events, and the error runs in both directions.** A corrected receipt is **edited in place, never superseded**, so: an item first proposed during an in-place re-vet is anchored to a receipt that predates intervening vets and can fire **early**; and **any** later in-place re-vet posts no new receipt, so the count does not advance — repeated re-vets can hold a long-carried item **below the threshold indefinitely, so it may never surface at all.** The rule computes the intended vet exactly only on the ordinary path: one new receipt per vet, no in-place corrections. This is **not** a narrow edge, and the tripwire must not be relied on as a guarantee that carrying gets caught. | The cause is structural, not editorial: vet receipts are per-PR comments that are **edited in place**, so they are neither a monotonic register nor a one-per-vet artifact, and a discrete ordinal is being derived from artifacts never designed to carry one. A fourth rewording would relocate the ambiguity again — the third-rework tripwire fired, so the seam is recorded for an owner design call instead. Named options, none taken here and none free of the in-place-edit problem: (a) the collector entry records a monotonic vet ordinal when it is appended, making age a subtraction rather than a count — the only option that survives in-place edits; (b) the tripwire keys on the item persisting across two **distinct** receipt artifacts — simpler, but it still fires late or never under repeated re-vets of one PR, so it is a partial fix and not the immunity an earlier draft of this row claimed; (c) the threshold becomes explicit advisor judgment with no arithmetic claim. | An escalation line is observed firing at the wrong vet, or failing to fire for a long-carried item, in a real receipt; OR the owner rules on options (a)/(b)/(c); OR a fourth rewording of the counting rule is attempted (take the design call instead). |
