...
| **Supervised write-dispatch control plane — per-invocation nonce on disk** (2026-07-29, advisor-ratified). Authentication for a supervised `dispatch-write` / `dispatch-review` spawn is a **nonce file held in the run directory**; a same-user **unconfined** engine CLI (`cursor -f` is not confined to `cwd`) can in principle read it. | The nonce raises forgery from *any stray file write* to *a deliberate, targeted act*. Launch data (`cwd`, `argv`, `orderId`, `attempt`, cleanup paths) binds to spawn arguments and is never re-read from the run dir for an authority decision; the lease path is derived, never read; the run dir is refused inside or equal to the write `cwd`. | That is the meaningful line for this threat model — steering via content, not a malicious local process. **Detection-over-prevention** for engine children remains the ratified stance (see the §3 worktree-confinement row — same trigger family, not restated here). | A field incident of a forged run artifact; OR Claude Code ships a native process-sandbox that can confine a `--trust` engine CLI (same re-check trigger as the worktree-confinement row). |
