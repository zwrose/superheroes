...
| **Native in-session seats are declared live and never probed** (2026-09-16). Unlike `codex` and `cursor` external-engine dispatches, native in-session seats — the review lenses, the grounding seat, the check-runner, synthesis, the pilot — are **declared** live rather than exercised before use. | Any certification that rests on an unprobed native family rests on a **declaration, not a receipt**; such a certification owes a disclosure line saying so. CONVENTIONS carries the population rule (`native in-session seats are declared live and never probed`); this row records the consequence. | Owner-accepted 2026-09-16 — probing every native seat on every dispatch would multiply cost and latency without a mechanical primitive to do it; `codex` and `cursor` are probed because their CLI contracts are external and variable. | A host primitive lets a plugin exercise native in-session seats before certification; OR a field incident where a native seat assumed live was demonstrably inert or misbehaving. |
