...
| **Dogfood plugin-vs-repository semantics skew detector (`lib/version_skew.py`, issue #677)** — detection only; the seat-map compose receipt always records `pluginVersionSkew` (`status`, `detail`, `inspectedRoot`) and emits one named `plugin-version-skew` degradation when `lib/model_registry.py`, `lib/seat_map.py`, or `lib/version_skew.py` diverges from the superheroes source repository — semantics-divergent or evidence-unreadable (only on `checked-degraded`). Named consumers: review-code's `seat_map compose` and `round_driver`'s certification shape | A plugin host that guaranteed the running plugin matched the repository under review, or that surfaced cache-vs-source skew itself | No host surfaces this today; the skew is invisible from inside a session and was caught four times only by operator vigilance (PRs #675/#679/#683). **Dogfood-specific:** a consuming project cannot skew — its installed cache is its only semantics source | A plugin host ships cache-vs-source skew reporting or version pinning of the running plugin to the repository; **or** the detector is observed firing on a case it cannot explain, or staying silent through an observed skew incident |
