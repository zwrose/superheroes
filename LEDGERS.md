...
| **"How many vets ago" is not reliably computable from the artifacts** (2026-07-29, #672/#694). The collector's escalation tripwire fires when an item was **proposed two or more vets ago**, and the charter tells the advisor to compute that age by counting vet receipts. Three successive attempts at the counting rule each fixed one boundary and exposed another (late by one; then an implicit anchor risking early by one; then an in-place-corrected receipt whose posting time predates intervening vets, which can fire early). | The tripwire is **directionally** sound — a long-carried item does surface — but the exact vet at which it fires is not guaranteed on every path. The known-bad path is narrow: an item first proposed during a **re-vet whose receipt was corrected in place** (the receipt contract forbids a superseding comment), so its anchor predates intervening vets. On the ordinary path — one receipt per vet, no in-place correction — the rule computes the intended vet exactly. | The cause is structural, not editorial: vet receipts are per-PR comments that are **edited in place**, so they are not a monotonic vet register, and a discrete ordinal is being derived from artifacts never designed to carry one. A fourth rewording would relocate the ambiguity again — the third-rework tripwire fired, so the seam is recorded for an owner design call instead. Named options, none taken here: (a) the collector entry records a monotonic vet ordinal when it is appended, making age a subtraction rather than a count; (b) the tripwire keys on something already immune to in-place edits — the item appearing in the pending set of two prior receipts; (c) the threshold becomes explicit advisor judgment with no arithmetic claim. | An escalation line is observed firing at the wrong vet (either direction) in a real receipt; OR the owner rules on options (a)/(b)/(c); OR a fourth rewording of the counting rule is attempted (take the design call instead). |
