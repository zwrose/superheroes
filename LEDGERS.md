...
| **Lane assignment is judgement with no mechanical check** (2026-07-26, spike #577, built in #671). The advisor calls the lane at routing (`rubric/review-discipline.md` carries the canonical lane statement); nothing mechanically verifies the call. | The advisor's **vet** catches a wrong call **before merge** — that is the only backstop, and it sits at the end. Nothing catches a wrong call **earlier**, at routing or during the build; the during-build escalation triggers depend on the builder noticing. | A mechanical lane rule is exactly the plugin-owned enforcement machinery §2 bans ("No plugin-owned enforcement machinery", owner-ratified 2026-07-20, spike #475), and the evidence could not support one anyway — the guidance was right about three times in four. | A defect **escapes through a light-lane PR the vet passed**; OR the vet formalization (#672) lands and changes the bound. |
