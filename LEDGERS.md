...
| **Pilot reclaim safety + reassignment fence (`lib/pilot_reclaim.py`, `lib/pilot_fence.py`), issue #824** — quarantine-never-delete with a crash-honest sidecar, a sweep that warns rather than reaps, deletion gated on a bound terminal receipt plus an elapsed 72-hour grace, journal rotation into retained segments, and the reassignment acceptance probe's grading and fail-closed retirement. Named consumers are the epic's successors — **C7** (broker-side fencing consumes the reassignment verdict), **B5** (stand-up/teardown invokes the sweep on acting runs), and **C8** (the owner-facing report reads sweep warnings) — so like the #822 and #823 rows this ships as pinned mechanism **ahead of its first caller, disclosed rather than claimed compliant** under CONVENTIONS §13. | No host primitive carries a quarantine store whose deletion predicate refuses a second read of the liveness marker that declared the occupant stale; the nearest in-repo thing is `lib/file_lock.py`, whose TTL-based stale reclaim is exactly that refused shape, and the nearest external thing is the reference project's `scripts/worktree-quarantine.js`, which is git-worktree-specific and does not generalize. | The design's own settled parameters — 72-hour grace, sweep on the next acting run, past-threshold warns, disk pressure never authorizes deletion — plus the LEDGERS heartbeat row's **3 watchdog design failures and 3 false alarms** from mtime and process-table signals, which are why those two are liveness sources that can never be terminal. | The epic's consumers land (C7/B5/C8) and the §13 residue closes, at which point the row is re-walked; **or** those consumers do not land and the unconsumed surface is retired rather than left to bit-rot; **or** a platform primitive appears that carries a crash-honest quarantine store with an independently-observed-terminus deletion gate and absorbs it. |
