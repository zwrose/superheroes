...
| **Supervised write-dispatch control plane — sealed `launch-authority.json` as cross-process trust root** (2026-07-29, owner-ratified Option A). The sealed `launch-authority.json` and its content hash are the trust root for a cross-process supervisor; a **same-user** unconfined engine (`cursor -f` is not confined to `cwd`) shares the filesystem with them (see the §3 worktree-confinement row — same same-user / unconfined-engine bound, not restated). | `run-child` accepts only a run-dir path and derives every authority value from the sealed file; the fold-time reference hash lives in a **supervisor-owned launch ledger outside the run dir** — a path never handed to the engine — and fold re-verifies the sealed file against that reference only (the receipt's hash echo is informational). The same relocation covers completion, retry-authorizing facts, the execution timeout, and the prompt digest. `O_EXCL` plus mode `0400` prevents *modification* but **not unlink-and-recreate** — which is precisely why hash re-verification, not file mode, is the mechanism, and why ledger records are **never deleted** (a canonical run-dir path is single-use). A swapped or recreated sealed file is **detected and the run refused at fold**. | **Owner-ratified 2026-07-29 (b)** ([PR #710](https://github.com/zwrose/superheroes/pull/710#issuecomment-5120685784)) as a bounded round on top of Option A ([#issuecomment-5116227979](https://github.com/zwrose/superheroes/pull/710#issuecomment-5116227979)): detection-over-prevention remains the ratified stance for engine children (see the §3 worktree-confinement row); the same-user residual stands unchanged — the relocation stops handing the reference to the thing being verified, not a defence against a same-user attacker. | A field incident of a forged or swapped run artifact; **or** a platform process-sandbox able to confine a `--trust` engine CLI (the same trigger the §3 worktree-confinement row names). |
