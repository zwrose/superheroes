...
| **Vet-checks section parser (`lib/core_md.py`: `parse_vet_checks`, `read_vet_checks`, `vet-checks`, `write-vet-checks`)** — strict parse of the optional `## Vet checks` prose section in `core.md`, the read verb, and writer refusal on malformed input. Named consumer: the showrunner vet (`skills/showrunner/reference/vet-receipt.md` § Project vet checks) and `configure`'s writer and view — shipped in the **same** PR | None today — no host primitive validates a plugin-owned calibration prose section | Without the strict parse a malformed section reads as "no checks" and the vet silently skips vet-carried rules; it enforces nothing mechanically (no hook, no CI gate, no PreToolUse boundary), so it is **not** a member of §2's plugin-owned-enforcement class. Owner-ratified precondition: the owner's verification-strategy adoption rulings (2026-08-27/28, receipts on #1105 and weekly-eats-hq/weekly-eats#1109), issue #1238 | A host primitive for project-calibration schema validation lands, OR the vet's vet-checks triggered field retires |
