...
Children: C1 founding texts · C2 front-door references (issue-contract.md, the rename, the census) · C18 the front door and the gardening pass (owner-decisions.md; split from C2 on the owner's word of 2026-09-16, walk item 8 = c) · C19 retire the legacy drift-tested copies (R3's completion; filed on the owner's word of 2026-09-16, walk item 13 = a; no acceptance row moves) · C3 review doctrine and one home · C4 committed-docs correction · C5 merge-gate retirement · C6 keep-or-retire rollout · C7 charter restructure · C8 calmer reviews · C9 background-session trial · C10 dispatch entry shrink · C11 native channels and the probe · C12 receipt writer · C13 certification loop · C14 CLI Claude engine and Astra · C15 orchestration decommission · C16 sanitized-view shrink · C17 configuration items.

**Sequencing.** *(Amended 2026-09-16: C18 lands in wave 2 after C2, its branch cut from C2's door draft; the door text is built once on the seam the owner settled at walk item 9.)* Wave 0: C1 alone, the seam; every other child cites its glossary and founding texts, so nothing launches until C1 has merged. Wave 1, after C1, on disjoint surfaces: C6 (the keep-or-retire doc, early so every later child can add its entries at birth), C9 (the trial, early so its receipt informs C15), C10, C16, C17. Wave 2: C2, C3, C4, C11 (after C10), C12 (after C6, so the writer's and checks' entries land in C12's own PR), C13 (after C12). Wave 3: C5 (after C3, same review-discipline file), C7 (after C2, C3, and C15: its one-hop targets and the orchestration that remains; a three-layer stack, one charter per layer), C8 (after C13), C14 (after C11), C15 (after C9's receipt and after C14: only one child at a time edits the launcher's spawn path, C14 retires the hand-built argv, then C15 removes detached spawn). The orderings the specs fix are honored: founding documents first, the trial early, CLI Claude after the dispatch shell, decommission after the trial's receipt, Astra after its probe (inside C14).

| Spec | FR | Bullet | Label | Criterion (opening words) | Owner |
| --- | --- | --- | --- | --- | --- |
| A | FR-A1 | 1 | Acceptance (rule, two doors; ruled 2026- | the door classifies every item at entry by FR-E1's machinery-versus-product test, the same test the… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A1 | 2 | Acceptance (rule): | "made to happen" means it broke in the field, or it was reproduced or probed in the lab. "Argued it… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A1 | 3 | Acceptance (rule, live vs dark): | a surface is live when it has at least one shipped consumer today: a shipped code path a shipped cal… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A1 | 4 | Acceptance (rule): | no severity thumb touches this bar. Every item needs executed evidence regardless of claimed severit… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A1 | 5 | Acceptance (rule): | an item that fails the bar is declined with a trigger (FR-A5), never silently dropped.… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A1 | 6 | Acceptance (rule): | lab evidence counts by design. Recorded rationale: the #1221 guidance-channel hole had zero field oc… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A2 | 1 | Acceptance (rule, impact instrument): | the severity ladder is a closed, owner-stamped enumeration of named bands with concrete examples in… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A2 | 2 | Acceptance (rule, urgency instrument): | urgency is the evidence tier (lab-only, field-once, field-recurrent), read directly off the FR-A1 ev… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A2 | 3 | Acceptance (record, surface heat deferre | the design artifact names a surface-heat adjustment without enumerating it. The owner deferred it to… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A2 | 4 | Acceptance (rule): | the grid stays two axes and a slot count. The framework acquiring its own machinery is the failure m… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A3 | 1 | Acceptance (rule): | a P1 or P0 grade waits on the owner's word to hold that tier. A cleared item may rest at P2 with no… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A3 | 2 | Acceptance (rule, the P2 carve-out): | the standing rule "no issue filing without the owner's word" (shipped as the owner-decisions contrac… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
| A | FR-A4 | 1 | Acceptance (rule): | every intake grading records its scoring durably on the item: band cited, evidence tier, resulting t… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A4 | 2 | Acceptance (rule): | a misses log captures three classes: declined-then-escaped, launched-then-regretted, and mis-tiered.… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A4 | 3 | Acceptance (rule, homes): | the misses log lives as a sibling section of the collector issue's pinned registry comment: one surf… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A4 | 4 | Acceptance (rule, the lane is a recorded | the routing record (FR-A9's routing-time calls, where the lane call is made) and every vet receipt n… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A5 | 1 | Acceptance (rule): | a declined item becomes a registry line with a named trigger: a concrete condition ("first real call… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A5 | 2 | Acceptance (rule, when triggers are read | a trigger gets exactly two detection moments, both existing acts. At the door, every new filing is c… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A5 | 3 | Acceptance (rule): | the worth-it gate's existing registry rows carry over unchanged. The gate's test retires at this spe… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
| A | FR-A5 | 5 | Acceptance (rule): | the registry has one home (Change 7). The collector's pinned revisit-trigger registry comment remain… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 1 | Acceptance (rule): | filing is cheap; launching is the guarded act. The budget is N machinery lanes in flight at once, co… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 2 | Acceptance (rule, the dial, precisely): | the dial is a ceiling on the machinery share of wave capacity. Over each gardening window, at most 2… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 3 | Acceptance (rule, which instrument binds | the dial governs; N is its per-wave planning expression. The accounting unit is the authorized wave:… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 4 | Acceptance (rule, a convention at the la | the dial is a planning convention, always discussable at the launch word. At wave planning the advis… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 5 | Acceptance (rule): | tiers order entry through the door; the owner's word sits at launch, where the capacity is actually… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 6 | Acceptance (rule, the reset is outside t | the landing epic (Part 6) is a one-time strategic reset. Its children, the Part C builds included, a… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A6 | 7 | Acceptance (rule, after the reset, no ex | from the reset's landing, every epic and milestone is product-forward and is labeled so by the owner… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A7 | 1 | Acceptance (rule): | at epic decomposition, the backlog and the declined registry are scanned for P1s, P2s, and registry… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A7 | 2 | Acceptance (rule): | an issue adopted into an epic counts as product-forward: it rides the package as ratified scope, not… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A7 | 3 | Acceptance (rule): | mid-build improvisation stays forbidden, exactly as today.… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A8 | 1 | Acceptance (rule): | any filing that proposes an instrument, detector, report, or sweep carries a reader clause: the name… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A8 | 2 | Acceptance (rule): | each project declares one unconditional digest as the floor: the minimum standing read that keeps "s… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A8 | 3 | Acceptance (rule, applied to the door it | the door's own instruments carry their reader clauses here. The misses log, declined registry, scori… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
| A | FR-A10 | 1 | Acceptance (rule, vocabulary): | a **ruling** is any decision the owner gives, in any session. A **decision walk** is any sitting in… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A10 | 2 | Acceptance (rule, trigger; ruled 2026-09 | the pass is owed seven days after the last gardening record, and it is done at an opportune sitting,… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A10 | 3 | Acceptance (rule, duties, closed list): | at each pass the advisor: 1. ages P1s: each P1 older than two passes is proposed promote, demote, or… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A10 | 4 | Acceptance (rule, the record's contents, | a gardening record carries: the batched words with their spines; the pending-words line (FR-A3); the… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A10 | 5 | Acceptance (rule, the record's shape; ru | the gardening record has no length cap. Each batched word carries the full per-item spine the owner-… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A10 | 6 | Acceptance (rule, windows are calendar d | the gardening window is the measurement window for the trend counts (forward share against the dial,… | **C18** (split from C2 on the owner's word, 2026-09-16) |
| A | FR-A10 | 7 | Acceptance (rule, the tripwire on the pa | when the owner says at any walk that the pass or its keep-or-retire list is taking too long, or an i… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
| A | FR-B3 | 14 | Acceptance (rule, the tripwire on the in | FR-A10 states it, in one place: when the keep-or-retire list outlasts the rest of the pass, or anyon… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
| A | FR-B5 | 5 | Acceptance (rule, at the door): | in-envelope variance is not defect evidence. A filing must show the shell was breached, or, only whe… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
| A | FR-B6 | 3 | Acceptance (record, quota retired outrig | quota exhaustion is no longer a thing the plugin watches. The usage poller is parked, not deleted. P… | **C22** (split from C15 on the owner's word, 2026-09-18; `harness_probe` and the re-arm request ride with it; R15's six pieces and the heartbeat stay with C15) |
...
| A | FR-C1 | 1 | Acceptance (rule): | forward share, the fraction of capacity making progress on new goals versus maintaining the machine,… | **split:** C1 owns the forward-share principle in PHILOSOPHY and the test-pilot classification convention (the recorded baseline moved to C17's profile item by Spec A amendment #4 on 2026-09-15); C18 (split from C2 on 2026-09-16) owns the decisions-asked count as a gardening-record line |
| A | FR-C1 | 2 | Acceptance (rule, classification convent | test-pilot work classifies as product; it exercises the product, not the machine. Folded-in adoption… | **split:** C1 owns the forward-share principle in PHILOSOPHY and the test-pilot classification convention (the recorded baseline moved to C17's profile item by Spec A amendment #4 on 2026-09-15); C18 (split from C2 on 2026-09-16) owns the decisions-asked count as a gardening-record line |
| A | FR-C1 | 3 | Acceptance (record, the baseline, polari | measured 2026-08-30 and corrected at CP3: outward-facing work is about 14% of open board stock and a… | **C17** (moved from C1 on 2026-09-15 by Spec A amendment #4: the baseline record lives in the configure profile's dial item, which C17 writes; the split with C2 on the decisions-asked count is unchanged) |
| A | FR-C1 | 4 | Acceptance (rule, the decisions-asked co | one count is recorded in every gardening record (FR-A10 duty 4), and nothing compares it yet: the **… | **split:** C1 owns the forward-share principle in PHILOSOPHY and the test-pilot classification convention (the recorded baseline moved to C17's profile item by Spec A amendment #4 on 2026-09-15); C18 (split from C2 on 2026-09-16) owns the decisions-asked count as a gardening-record line |
...
| B | FR-M2 | 4 | Acceptance (rule, a tripwire firing sepa | every tripwire this spec names (FR-D3, FR-D6, FR-D9, FR-S1, FR-S7) is a prompt for an owner conversa… | **C18** (split from C2 on the owner's word, 2026-09-16) |
...
**Re-check 2026-09-15 (package fix after the stamp; owner-ruled at the review of PR #1277):** FR-B6 bullet 6 (the sentence layer, `rubric/prose-standard.md`) moves from C3 to C1 so the standard lands with the founding texts it is first applied to; C3 keeps bullets it owned elsewhere unchanged. Spec A amendments #1 to #3 (FR-F1 twice, FR-F9) touch criteria owned by C1 only; amendment #4 (FR-C1) moves the baseline record's row from C1 to C17; every criterion stays owned once. Register entry R29 (no project provenance on shipped surfaces) added with sixteen consumers.
