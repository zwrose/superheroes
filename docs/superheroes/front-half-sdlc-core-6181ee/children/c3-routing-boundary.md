...
**Anchor:** spec `front-half-sdlc-core-6181ee` §Routing at intake (FR-5, FR-6) + FR-11's routing acceptance + §The builder's side of the line (FR-39) — owner-approved 2026-08-07, as-of amendment #4.

**What:** The advisor's intake becomes four named routes per R6's judgment-input form
(FR-5 amendment #4: overlaps resolve by recorded advisor judgment — no precedence
procedure exists or ships) — with this child owning route selection EXCLUSIVELY, including FR-11's routing outcomes (a
...
**DoD:**
- Showrunner charter: the four routes with R6's judgment-input text verbatim + FR-5's
  overlap-resolution rule; micro re-route rule stated; FR-11's two routing outcomes exercised once each with RECORDED routing receipts in the
  handback (ruled follow-up → build-ready; unanswered product question → discovery).
