...
- Workhorse charter: intake resolution with the three per-kind tests quoted from R1 —
  ...
  exercised once against a stale anchor (spec DoD's intake-stop leg) — no file changed,
...
- Intake invokes R3's exact-text check where a child issue carries a register block — this
  child owns ONLY the intake invocation (R3's split: C9 ships the checker; C6 owns filing
  and verification).
