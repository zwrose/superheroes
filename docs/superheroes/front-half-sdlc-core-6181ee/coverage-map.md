...
each FR/UFR group marked as faces of one behavior allocates as one. Three declared splits,
...
Round 2 un-split FR-31 (whole to C9) and consolidated FR-22 + UFR-4 (whole to C6).
...
**Sequencing (FR-30 + findings B4/B2r2):** C1 first (R1/R2 + the NFR vet row are the
seam), then C9 (the whole exact-text criterion, including its charter invocation lines —
this package's own filings run under R3's disclosed advisor bootstrap until C9 ships).
C2–C5 launch after C9 with charter-file overlaps sequenced at launch (disjoint pairs
parallel). **C6 launches only after C2 and C3 merge** (shared showrunner vet-duty and
workhorse-intake surfaces). C7 after C6. C8 last. The detective child (sibling spec) is
ordered freely per R9's landing rule.
...
| FR-11 front-door rule (one door; no spike surface) — the ruling-bypass ROUTING outcome is C3's | FR-11 first rule | **C4** *(narrowed — round-2 finding C5r2)* |
...
| FR-22 (amendment classes, in-place edit, log format) + UFR-4 (mid-flight propagation, all elements, both seam sides) — one behavior, one owner | FR-22 both bullets; UFR-4 bullet, all elements | **C6** *(consolidated — round-2 finding C4r2)* |
...
| FR-30 (seam first) | FR-30 bullet | **C6** |
| FR-31 (verbatim quoting + the machine check + ALL THREE invocation points) — whole criterion, one owner | FR-31 rule, whole | **C9** *(round-2 findings A3r2/C3r2; C2/C6 are dependents only)* |
...
| FR-40 (adoption pass) | bullet + rule | **C8** — declined 2026-08-15 (#939 NOT_PLANNED); calibration-record leg removed by Amendment 2026-08-21 |

## Non-functional requirements — package-wide

The three NFRs (owner reading load; plain language; guidelines over trip-lines) are
constraints on every child, graded at each child's vet via **the standing NFR vet row C1 ships** (moved from C6 —
round-2 finding C6r2: C1 merges first, so every later child's vet has the row), and
re-checked in the closure receipt's NFR element (R8). A child introducing an owner gate that reads outside the NFR's artifact
...
- **2026-08-08 (round-2 fold):** FR-31 un-split, whole to C9 (A3r2/C3r2); FR-22+UFR-4
  consolidated to C6 (C4r2); FR-11 bypass-routing wholly C3 (C5r2); NFR vet row moved to
  C1 (C6r2); bootstrap note in sequencing (B2r2); anchors carry the ordered as-of cursor
  (A4r2/B3r2/C1r2).
