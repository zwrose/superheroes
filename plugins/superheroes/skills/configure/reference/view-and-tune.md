...
- **Pin a concrete Codex model for one role** → keep the provider-neutral `## Model tiers` block
  ...
  model IDs are `gpt-5.6-terra`, `gpt-5.6-sol`, and `gpt-6-astra` (probe-pending — a valid pin for
  `reviewer-deep` only after its registration probe has passed; until then the pin is refused
  `pin-probe-pending`, and pinning it on any other role is refused `pin-role-not-eligible`). Codex
