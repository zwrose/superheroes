...
- **Pre-authorize owner-judgment review gates** → write a narrow `gate-policy/1` overlay under
  ...
  nonblank `item`. A `followUp` that is present but malformed, or attached to any other rule, is
  refused when the overlay loads and when the calibration write is checked. Only a rule with **no**
  `followUp` resolves without one — the driver records the disposition and certification refuses it
