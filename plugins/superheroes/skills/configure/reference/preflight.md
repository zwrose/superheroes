...
stamped `probed` provenance — per-cell probe evidence, fresh on a cache miss or reused from
a within-TTL receipt) plus the `liveVendors` audit rollup (`review-code` passes the
