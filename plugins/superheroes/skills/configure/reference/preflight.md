...
When the readout is a single `role: "*"` marker row carrying `readError`, carry that refusal
verbatim into the brief and the PR — never substitute defaulted engine/model rows, because the
preferences were never read.
