...
**dependency-cruiser TypeScript compat.** dependency-cruiser silently degrades to JavaScript-only parsing when the resolved TypeScript major is outside what it supports. Measured on a real Next.js repo: with TypeScript 6.0.3 resolvable it returned **2 modules against 590 TypeScript sources**, exit 0, empty stderr. **TypeScript pin provisioning is deferred** (the shared `run_tool` seam does not forward `extra_node_path`). The **module-count-collapse tripwire is the compensating control**: on a host whose depcruise resolves TypeScript ≥6 the parse silently drops, and the tripwire must degrade honestly (`not-collected` with a naming reason) rather than write a false-clean baseline. The digest's `versions` block still records resolved tool/TypeScript versions, `parseMode`, and informational `pinHeld` when the report carries them.
