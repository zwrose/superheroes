...
emits the stable `"<ecosystem-or-lens>/<part>/unclassified-partial"` **sentinel** rather than poisoning `identity` to a non-comparable state — an unclassifiable partial over-alerts (stays comparable), it never silences the drift comparison (#592, fail-direction ruling). Two **transient cause** markers — a malformed advisory row (`malformed-advisory`) and an unreconciled ambiguity (`ambiguous-identity`) — are ecosystem noise that can flip between sweeps with no repo change; they are folded **out** of the `identity` key (comparability keys on stable causes only) and surfaced in the human-readable `reason` instead, so a flip never makes two otherwise-identical partial bases read non-comparable and silence a real crossing on a co-present stable marker (#613). The sweep that first runs a changed cause-token scheme (deploying or rolling back this fold-out) compares a new-scheme `identity` against the prior sweep's persisted old-scheme `identity` for any partial basis that carried a transient token, so that one crossing is recorded non-comparable and silenced for a single, self-healing sweep — a bounded transition tax accepted rather than guarded, because normalizing persisted identities on read would make `guardian_vitals` hardcode a lens's cause vocabulary (breaking the lens↔vitals split) and still miss transient-only priors; steady state stays over-alert, never silence (#613). ("Transient cause" here is distinct from the collector-version *transient partial* sense below.) A 2-tuple partial (no identity) is treated as non-comparable (fail-closed) at the `guardian_vitals` layer, reserved for malformed completeness *entries* — a contract violation, not an unclassified basis.
