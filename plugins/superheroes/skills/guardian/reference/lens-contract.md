...
Every production lens MUST implement `conformance_cases()` covering all names in `guardian_lens.REQUIRED_CONFORMANCE_SCENARIOS` (CONVENTIONS §11 — that tuple is the scenario roster's one home). The per-lens conformance suite (`test_guardian_conformance.py`) runs against every registered lens and **fails registration** when a scenario is missing or an honesty invariant breaks: degraded tool outcomes (`missing-tool`, `timeout`, `nonzero-exit`, `findings-empty-output`, `unparseable`, `reported-nonzero-parsed-zero`) must never read as `collected`; a tool that reported problems must never yield `collected` with zero candidates; and when collection stops, `diff()` must never emit `resolved` ids.
