...
1. **Collect (deterministic).** Registered lenses run standard OSS collectors with thin normalization. Each lens diffs its digest against the prior snapshot and surfaces only new or worsened candidates. Red lines (absolute thresholds) bypass the baseline quiet rule. On a lens's first-ever baseline, candidates route through the same one model validation pass — high-precision lenses validate all candidates up to `firstBaselineValidateMax` (default 10), volume lenses only up to that bound, with any remainder quiet-baselined and the skipped count disclosed; a collector-version re-baseline on an already-seeded lens stays quiet.
...
**Drift-over-baseline.** On a lens's first-ever baseline, high-precision lenses route candidates through validation (up to `firstBaselineValidateMax`, default 10); volume lenses validate only up to that bound and disclose the rest as baselined unreviewed; a collector-version re-baseline stays quiet. Later sweeps surface only what changed since the last snapshot.

**Lens rollout.** The health lenses (duplication, complexity, coupling, dependency and doc freshness, dead code) land across the guardian arc. The registered lenses today are <!-- guardian:registered-lenses:start -->`duplication`, `hotspots`, `deps`, `deadcode`, `docs`, and `coupling`<!-- guardian:registered-lenses:end --> (complexity/hotspots; `deps` covers dependency freshness + known vulnerabilities with check-the-check for a covering tool — Python advisories are severity-rated via osv-scanner so the critical-vuln red line covers Python, degrading to unrated pip-audit when the scanner is absent; `deadcode` runs vulture on Python + knip on Node for unused symbols, files, and exports — validated hard, since most hits are conventions the project sanctions on purpose; `docs` is stdlib-only and genuinely tool-free — it resolves the paths that root instruction docs and the calibrated verify command name, so a doc that points a future agent at a moved or deleted path surfaces, and it NEVER runs the verify command; `coupling` is a single lens covering JS/TS via dependency-cruiser and Python via an AST import census — data-only while declared-vocabulary reading is owner-deferred — so its baseline drift is counter-level (the `couplingEdges` vital on the snapshot), not edge-level: with vocabulary deferred the digest's `eligible` index is empty, so lens diff surfaces no edge drift until vocabulary reading is un-deferred); the remaining lenses land as their orders merge — a sweep today runs the registered lenses with first-baseline validation as above. Each later registration adds its lens name between the markers above **and** to `guardian_lens.PRODUCTION_LENS_NAMES` in the same PR; the roster sync guard fails closed if they drift.
