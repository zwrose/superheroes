...
**Lens rollout.** The health lenses (duplication, complexity, coupling, dependency and doc freshness, dead code) land across the guardian arc. The registered lenses today are <!-- guardian:registered-lenses:start -->`duplication`, `hotspots`, `deps`, `deadcode`, and `docs`<!-- guardian:registered-lenses:end --> (complexity/hotspots; `deps` covers dependency freshness + known vulnerabilities with check-the-check for a covering tool; `deadcode` runs vulture on Python + knip on Node for unused symbols, files, and exports — validated hard, since most hits are conventions the project sanctions on purpose; `docs` is stdlib-only and genuinely tool-free — it resolves the paths that root instruction docs and the calibrated verify command name, so a doc that points a future agent at a moved or deleted path surfaces, and it NEVER runs the verify command); the remaining lenses land as their orders merge — a sweep today runs the registered lenses and stays quiet on a first baseline except absolute red lines. Each later registration adds its lens name between the markers above **and** to `guardian_lens.PRODUCTION_LENS_NAMES` in the same PR; the roster sync guard fails closed if they drift.
