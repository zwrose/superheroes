...
| `rounds` | Per-round projection with disclosure channels via `receipt_disclosures`. Each round may carry `controlProbe`: `{"submitted": <bool>, "vendors": {<engine>: <outcome token> \| "malformed"}}` — the sampled competence-probe summary (`ok`, `plant-undetected`, `not-engaged`, dispatch-failure tokens, or `"malformed"`; `submitted: false` with empty `vendors` when no probe ran). Recorded for disclosure; the certification writer never reads it into a verdict. Absent on older rounds means unrecorded. |
