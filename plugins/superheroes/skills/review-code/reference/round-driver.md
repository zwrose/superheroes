...
| `round-diff-head-mismatch` | the round-1 diff is not byte-for-byte the review diff the driver derives at the checkout's HEAD (HEAD moved since Setup, or the diff did not come from the `review-diff` verb), or the head `meta.json` recorded (in PR mode, the PR's `headRefOid`) is not that HEAD — the checkout is behind or ahead of the head under review |
| `review-diff-unavailable` / `review-diff-too-large` | the driver could not derive the review diff at HEAD to bind the round-1 diff to it (past the size cap for the second) |
...
- **Diff artifact** (`round-diff-required`, `round-diff-unreadable`, `round-diff-empty`,
  ...
  `round-diff-head-mismatch`, `review-diff-unavailable`, `review-diff-too-large`,
  ...
  `round-diff-base-unverifiable`, fix git availability/timeouts before re-running `next`; for
  `round-diff-head-mismatch`, bring the checkout to the head under review (in PR mode, the PR's
  `headRefOid`) and re-run Setup so the diff and the recorded head are taken together.
...
| `dispatch-fixer` | Dispatch fixer over `payload.batch` (blocking findings the driver selected). Submit `{fixes, headDiff \| headDiffPath, escalated?, coverageDecisions?}` — `coverageDecisions` is a list of coverage-decision objects the driver accumulates into `state["_coverage"]`. The post-fix head diff comes from git via the **`review-diff` verb** the SKILL's Setup runs (`round_driver.py review-diff --base "$BASE_REF"` against the **pinned remote base commit** — never a local branch name; if `$BASE_REF` is not in this shell, restore and re-validate it first, #637), never the fixer's self-report. Provide it **inline** (`headDiff`) or, since a real head diff can be hundreds of KB and cannot reasonably inline into a JSON submit artifact, as an **absolute** file path (`headDiffPath`) (**inline wins if both are present**); the source supplied is recorded on the round as `headDiffSource: inline\|path\|unknown`. The **unknown surface** is keyed on the driver's own derivation alone: when no diff at the post-fix head is derivable from git, the next round runs a full reviewer-deep panel (the unknown→run-everything rule), never an empty diff and never a silent scoped skip. **Git is the authority for the diff a panel reviews.** At every fixer fold the driver derives the review diff itself at the fix-fold head — the same command SKILL.md's Setup runs, `round_driver.py review-diff` (its one home; config pins and env hardening included; admitted only as non-empty strict UTF-8 opening with `diff --git `; recorded as `reviewedDiffSource: git-derived`) — and binds the reviewed diff to the SHA it was derived at; certification compares that SHA with the certified fix-fold head, so a commit landed after the fold is never certified unseen. **The certified head is recorded by the call that derives the diff:** one driver function resolves HEAD to a SHA once and derives the diff at that explicit SHA, and the pair (SHA, diff digest) is persisted together — at the fresh `next` for the round-1 diff, and at every fixer fold. A certificate names that recorded SHA (`certifiedHead`) and nothing else: no config, meta or live-HEAD fallback. A converged terminal with no recorded head (a session an older driver minted, schema 2–5, or a malformed record) parks `cannot-certify` with `reviewed-head-unrecorded`, recoverable by a fresh session; only the in-process `run_loop`, which has no repository, certifies without one. The terminal sidecar of a converged session publishes `certifiedHead` and refuses `reviewed-head-unrecorded` when the certificate names none. A supplied `headDiff`/`headDiffPath` carries no authority over the reviewed diff: it is read only to tell a supplied surface from an unknown one (`headDiffSource`). A session that cannot derive (no pinned base, no repository, or `run_loop` with no derivation seam) has no head diff at all. **The invariant lives at certification:** a converged terminal is never written while the reviewed diff is not bound to the certified head — the session parks `cannot-certify` with `reviewed-diff-stale`, recoverable by a fresh panel — whatever the route (a legacy resume, a hand submit, any later path). Earlier exits read the same rule: panel-order emission (`next`, `advance`, `re-emit`), an already-pending panel order about to be replayed or folded (`next`, `advance`; a hand `submit` folds and records the panel's output first, then parks), and `run_loop` before a panel. The changed policy subjects the #174 confirmation re-arm consumes are **derived by the driver itself** from the reviewed-vs-head diff through the accumulated findings (the injectable `changed_subjects` seam — library default + CLI wire the real git derivation, #157/#158); a self-reported `changedSubjects` is ignored on the live path. **Batches.** `payload.batch` carries at most `fixBatchCap` findings (default per § Round economy); a larger blocking set is dispatched as consecutive `dispatch-fixer` attempts in the same round (`fix-batch.json`, `fix-batch.1.json`, …), each folded on its own, and the round's audit targets derive from the whole batch. The emitted order's verify line is the **scoped budget** (`{{VERIFY_BUDGET}}` — the batch's target files, their referencing tests, the static validators), never the full verify command. |
