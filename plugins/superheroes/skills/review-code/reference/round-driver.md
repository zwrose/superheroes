...
- [Seat-time runner proof](#seat-time-runner-proof)
...
manifest, so it finds those stale files. It detects **both** landing shapes — a host bare payload
`<skey>.a<K>.payload.json` as well as a full envelope `<skey>.a<K>.json` — and one stray storage
...
(`record-result --seat <key>`) that is not on the current roster — not the sweep's stray-file
refusal; a reader must be able to tell the two apart from this doc alone.

**`record-result --sweep` cannot supersede.** Combining `record-result --sweep` with `--supersede` or
...
On a durable-record session (`seat-result/2`, folding through `advance`) a certification rests only
on runner records, so a seat that cannot produce one is refused **when its order is emitted**, never
when it lands. Every seat of `dispatch-panel`, `dispatch-verifiers`, `dispatch-scoped-finder`,
`dispatch-gap-sweep` and `dispatch-audits` must be on a vendor the orchestrator dispatches through
the runner (codex, cursor — a claude seat is a host seat on this driver path). Any other vendor, or
none, refuses the whole emission with `order-render-refused:<skey>:seat-no-runner-record:<vendor>`
before any dispatch; recover with a fresh session whose seat map and `--vendors` name codex or
cursor. Synthesis stays the one host seat: its unproven landing is the named `unrun-review` that
ends such a session `cannot-certify`. The round-1 panel is emitted before the first fold chooses the
session's path, so it is not gated here; a host seat there without execution evidence still
refuses `unrun-review` at certification. A hand-`submit` session keeps its old seating.

**Dispatch the seat the manifest names.** On a durable-record session the driver selects the vendor,
model, effort and role of every `dispatch-verifiers`, `dispatch-scoped-finder`, `dispatch-gap-sweep`
and `dispatch-audits` seat and writes them to that seat's entry in
`$SESSION_DIR/round-N/orders/<phase>/manifest.aK.json` (`seats.<skey>`: `vendor`, `model`,
`effort`, `role`). Build each seat's `dispatch-review --seat` bundle from that entry — never from
`$REVIEWER_ENGINE` / `$VERIFIER_MODEL`, which may name a host vendor the driver refused to seat, and
never from `$SEAT_MAP`, which holds only the panel. A seat dispatched on any other vendor lands a
runner record that does not match the seat the driver selected.
...
- The state marker `dispositionLedgerOwner: "ledger"` tells the certification writer that the
  **disposition ledger is the durable owner of record** — the writer's finding set is seeded from
  ...
  recorded shape. No stored field is removed and there is **no state-schema version bump**. Any
  other marker value (including `null`) is **unrecognized**: `next`, `re-emit` and `advance` refuse
  to hand out a non-terminal step (`disposition-ledger-owner-unrecognized`, journaled), so no seat
  is dispatched for a fold that would refuse; `submit` and the fold refuse it too.
...
| `dispatch-verifiers` | One verifier per cluster in `payload.clusters` (`model: $VERIFIER_MODEL`, reviewer engine; on a durable-record session, the cell its orders-manifest entry names — § Seat-time runner proof). Submit `{verdicts: [...]}`. **`payload.clusters` is an independent batch — dispatch its verifiers concurrently, per § Batch concurrency above; submit the phase once, with every verdict in the one artifact.** |
