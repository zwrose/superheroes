...
A synthesis artifact with **no `grouping` key** is **refused at submit** by `synthesis_results_fault`,
naming the missing key (resubmit with `{"grouping": null}` or a group list); a `null` or empty-list
`grouping` carries no coverage obligation and falls open to unmerged survivors — synthesis drops
nothing. `verification.grouping_coverage_fault` enforces the synthesis **coverage guarantee** for
non-empty groupings: a **non-empty** grouping must account for every staged survivor id exactly once; a grouping that
