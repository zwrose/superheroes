...
survivors. A synthesis artifact with **no `grouping` key** is **refused** at the submit chokepoint
(`synthesis_results_fault`: "synthesis artifact carries no `grouping` key") — absent is not null — **synthesis drops nothing** and synthesis failure never aborts the review.
