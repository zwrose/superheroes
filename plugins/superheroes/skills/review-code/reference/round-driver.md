...
- **Base / pin / repo / checkout** (`base-meta-unreadable`, `base-not-pinned`, `base-unresolved`,
  ...
  repository at the same head and base, `relocate` moves it (see [Moving a session — relocate and
  re-emit](#moving-a-session--relocate-and-re-emit)) instead of starting over.
...
and findings. `relocate` and `re-emit` move the session instead.
...
| `reason` | condition |
| --- | --- |
| `relocate-session-unreadable` | meta or loop state missing/unparseable or the session never ran `next` |
...
| `relocate-same-checkout` | the target is the recorded checkout and the session directory has not moved |
...
**re-emit** acts only on the session's pending `dispatch-*` order. An order is stale when a
`relocated` row that actually moved the root or the session directory comes after that order's
`orders-emitted` row. It re-renders the order as the next attempt — the old attempt's orders,
stubs, manifest and anchor are left byte-identical — and journals, in one transaction, an
`orders-superseded` row (old attempt, new attempt, the superseded manifest and order hashes, the
head, the relocation that made it stale, `by`, `at`) followed by the new attempt's `orders-emitted`
row. Certification treats a superseded attempt's seats as closed and the new attempt's seats as open.
Dispatch the new attempt's orders; results recorded against the old attempt are not carried.

| `reason` | condition |
| --- | --- |
| `re-emit-session-unreadable` | loop state missing/unparseable |
| `re-emit-no-pending-order` | no pending order, it is not a dispatch order, or the session is terminal |
...
| `re-emit-not-stale` | no relocation that moved anything came after the order was emitted |
| `re-emit-attempt-has-results` | a result for the pending attempt is already recorded or has landed |
...
the session's recorded head. `re-emit` does not change the session's recorded head, so an order
