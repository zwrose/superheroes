...
| `auditor-unseatable` | on a durable-record session, `next` would emit `dispatch-audits` but no runner-backed vendor (`codex`/`cursor`) is live to seat the fix auditor |
