...
| `canaryOutcomeFailed` | `canaryResult` was submitted and engaged but the probe outcome is a dispatch failure (`forfeited`, `vacuous`, `forfeit-with-engaged-artifact`, `unrunnable`, or `entry-reason-undeclared`) — the seat stays `run` but panel certification is withheld. A seat whose dispatch returns `entry-reason-undeclared` was refused at entry because its refusal reason was outside the declared entry vocabulary — the seat did not run and is treated as a dispatch failure like the other not-run reasons. | `canary-outcome-failed (round N): …` |
