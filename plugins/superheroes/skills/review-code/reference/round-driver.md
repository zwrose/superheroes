...
| Canary probe | `$SESSION_DIR/round-N/landing/dispatch-panel/_canary/vendor.aK.json` | **Orchestrator** (`seat_canary.py probe`) — panel phase only, on **sampled** reviews (see **Cross-vendor control probe** in `auto-fix-loop.md`). Carries the control-probe result `advance` folds as `canaryResult` and records on the round as `controlProbe`. **Absence:** no landing file — on a non-sampled review that is expected; on a sampled review with no submit, `controlProbe.submitted` is false. The probe is recorded and **never a gate**. |
...
| `controlProbe` | Panel fold wrote the sampled competence probe summary: `{"submitted": <bool>, "vendors": {<engine>: <outcome token> | "malformed"}}` (normalized outcome tokens: `ok`, `plant-undetected`, `not-engaged`, dispatch-failure outcomes, or `"malformed"` for an unreadable entry; `submitted: false` with empty `vendors` when no probe was submitted). Recorded on the round; never read into certification or seat status. | *(none — recorded on the round)* |
| `canaryUnverified` | Sampled panel context expected a probe and none was submitted (legacy disclosure channel). | *(none — recorded on the round)* |
