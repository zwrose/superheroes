...
verify command's `{baseRef}` token (#1336, bound by the driver to the pinned base) keeps a stacked
branch's gate scoped to its own layer.
...
**default 4**, positive integer — refused `fix-batch-cap-invalid` otherwise) limits findings per
