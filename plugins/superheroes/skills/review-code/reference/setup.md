...
COMPOSE_ERR=$(mktemp)
if ! SEAT_MAP=$(python3 -B "$ROOT_DIR/lib/seat_map.py" compose --configured-engines "$CONFIGURED" --implementation-engine "$IMPL_ENGINE" --host-model "${SUPERHEROES_HOST_MODEL:-}" --pr-number "${PR_NUMBER:-}" --head-sha "$(git rev-parse HEAD 2>/dev/null)" "${PINS_ARGS[@]}" --repo-root "$REPO_ROOT" 2>"$COMPOSE_ERR"); then
  echo "seat-map-compose-failed: $(cat "$COMPOSE_ERR")" >&2
  rm -f "$COMPOSE_ERR"
  exit 1
fi
rm -f "$COMPOSE_ERR"
