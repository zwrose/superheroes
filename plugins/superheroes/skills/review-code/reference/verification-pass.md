...
singleton groups. A **null** or **empty-list** grouping carries no coverage obligation (a
submitted artifact with **no `grouping` key** is refused at submit — send `{"grouping": null}`): `verification.merge_and_rank(survivors, grouping)` falls open to unmerged survivors;
