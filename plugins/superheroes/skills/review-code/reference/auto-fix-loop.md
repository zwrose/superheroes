...
| **Continuing when `$BASE_REF` is empty**                | `git diff "$BASE_REF"...HEAD` with an empty `$BASE_REF` is argv `...HEAD` — git reads it as `HEAD...HEAD` and emits a **zero-line diff at exit 0**, so the panel reviews nothing and the loop certifies clean. The setup block's non-empty guard HALTS instead; never bypass it, and never substitute a branch name to "recover" (#637). |
...
| Using `gh pr diff` inside the loop                      | Rounds 2+ have local fix commits not on the remote. Always recompute `git diff <baseRef>...HEAD` locally each round, against the **pinned remote base commit** the SKILL bootstrap resolved — never a branch name.                                               |
