...
| Landing — **host** seat (`claude` native subagent) | `$SESSION_DIR/round-<N>/landing/<phase>/<skey>.a<K>.payload.json` — write **only** the payload; the driver wraps with the emitted stub at ingest. At state v5 the ingested envelope is `seat-result/2`; when execution evidence is stamped the bare-payload landing stays single-file and the stamped envelope is written to the durable store copy only. A host seat can never carry a runner record, so on a durable-record session (state v5, folding through `advance`) the driver seats panel, verifier, scoped-finder, gap-sweep and audit seats only on a runner-backed vendor (`codex`/`cursor`). Any other seat refuses at emission with `order-render-refused:<skey>:seat-no-runner-record:<vendor>`, before any dispatch. Recover with a fresh session whose seat map and `--vendors` name `codex` or `cursor`. Synthesis stays the one host seat, and no host seat without execution evidence ever certifies (`reference/round-driver.md` § Seat-time runner proof). A hand-`submit` session keeps its old seating and does not apply this gate. |
