...
8. **PR-body honesty check (PR mode only).** Two legs — full contract: `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}/skills/review-code/reference/grounding-seat.md`. **Leg 1 (dispatched):** `grounding_stage stage|check|attest`, then dispatch the grounding seat (`$SEAT_MAP.seats["grounding-seat"]`); each `REFUTED` repo-verifiable claim becomes an orchestrator-minted Important finding. Any `grounding_stage` refusal (`{"ok": false, "signal": "cannot-certify", ...}`, exit 1) **halts the review** — never proceed. **Leg 2 (retained inline):** the orchestrator-inline PR-body honesty check — from `pr.json`'s `body`, verify DoD disposition table (`superheroes:dod-table` marker), `<!-- superheroes:build-record -->` boundary, `<!-- superheroes:degradations -->` section (bullets, or **None** when empty; marker absence and **None** differ), omission floor under `## What we're accepting` — enumerate (1) each deferred DoD row, (2) each blocking or important dispositions-table finding not fixed (by severity, not disposition label), and (3) each disclosed degradation under `<!-- superheroes:degradations -->` (bullets, or **None** when empty) — CONVENTIONS §10.7; covers `external`-verifiability claims the sanitized-view seat cannot settle. Branch mode — skip both legs.
