...
8. **PR-body grounding (PR mode only).** Two legs — full contract: `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}/skills/review-code/reference/grounding-seat.md`. **Leg 1 (dispatched):** `grounding_stage stage|check|attest`, then dispatch the grounding seat (`$SEAT_MAP.seats["grounding-seat"]`); each `REFUTED` repo-verifiable claim becomes an orchestrator-minted Important finding. Any `grounding_stage` refusal (`{"ok": false, "signal": "cannot-certify", ...}`, exit 1) **halts the review** — never proceed. **Leg 2 (retained inline):** the orchestrator-inline PR-body honesty check (DoD table, build-record/degradations markers, omission floor — CONVENTIONS §10.7) covers `external`-verifiability claims the sanitized-view seat cannot settle. Branch mode — skip both legs.
