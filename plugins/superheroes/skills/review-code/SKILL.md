...
- **`--review-only`** → one review pass, then the presentation written to `$SESSION_DIR/round-1/presentation.md`. No commits. No questions that block waiting for an answer.
...
| `/superheroes:review-code --review-only`   | One review pass, presentation written to `$SESSION_DIR/round-1/presentation.md`, no commits. Never a stall on a question nobody can answer.       |
...
**Resolve the run's inputs before dispatching anything.** In order: the base rubric path, repo root, the calibration paths, the plugin and rubric versions, the model tiers, the per-role engine, the panel seat map, then the staleness self-check, the profile bootstrap, the verify story, and the post-bootstrap refresh of the dispatch paths. The exact commands, the variable each one sets, and the tier→model dispatch mapping are in `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}/skills/review-code/reference/setup.md` § Setup resolution — read it and run the blocks in the order given.

Everything below depends on the variables that file sets: `$ROOT_DIR`, `$RUBRIC`, `$REPO_ROOT`, `$CORE`, `$LAYER`, `$PROFILE`, `$LOCATION`, `$EXISTS`, `$DECISIONS`, `$PLUGIN_VERSION`, `$RUBRIC_VERSION`, `$REVIEWER_MODEL`, `$DEEP_MODEL`, `$MECH_MODEL`, `$SYNTH_MODEL`, `$VERIFIER_MODEL`, `$FIXER_MODEL`, `$EP`, `$REVIEWER_ENGINE`, `$IMPL_ENGINE`, `$CONFIGURED`, `$AUTHOR_FAMILY`, `$SEAT_PINS`, `$PINS_ARGS`, `$SEAT_MAP`, `$DOCTOR_JSON`, `$VERIFY_JSON`, `$VERIFY_CMD`, `$VERIFY_MODE`, `$REFUSAL`. A step below that reads one of these without that file having been run is a bug in the step; the shell defaults that do appear (`${VERIFY_CMD:-unverified}`, `${VERIFY_CMD:-none}`) are deliberate value fallbacks for an unset-but-legitimate verify story, not a licence to skip the setup file.
...
```bash
git -c core.commitGraph=false -c core.quotePath=false -c diff.noprefix=false -c diff.mnemonicPrefix=false -c diff.relative=false diff --no-color --no-ext-diff --no-textconv "$BASE_REF"...HEAD > "$SESSION_DIR/round-<round>/diff.txt.tmp" && mv "$SESSION_DIR/round-<round>/diff.txt.tmp" "$SESSION_DIR/round-<round>/diff.txt" || { rm -f "$SESSION_DIR/round-<round>/diff.txt.tmp"; echo "review-code: git diff against $BASE_REF FAILED — refusing to review the artifact it left behind (#637)" >&2; exit 1; }
...
- **Path:** default → auto-fix loop (`round_driver.py` next/submit until terminal); `--review-only` → one pass, then the presentation written to `$SESSION_DIR/round-1/presentation.md`
...
| Landing — **engine** seat (`codex`/`cursor`) | `$SESSION_DIR/round-<N>/landing/<phase>/<skey>.a<K>.json` — **orchestrator** writes the **full envelope** (stub header + payload; schema from state version) from the folded `dispatch-review` stdout result; the engine seat emits JSON on stdout only (read-only sandbox). At state v5, pass a completed `dispatch-review` run directory to `record-result --evidence-run-dir` so the driver can stamp `executionEvidence` and `envelopeSha256` before ingest. For a fix auditor's `ruling` result, the orchestrator may land the stub envelope with `payload: null`, and `record-result --evidence-run-dir` fills it from the runner's graded record. |
| Landing — **host** seat (`claude` native subagent) | `$SESSION_DIR/round-<N>/landing/<phase>/<skey>.a<K>.payload.json` — write **only** the payload; the driver wraps with the emitted stub at ingest. At state v5 the ingested envelope is `seat-result/2`; when execution evidence is stamped the bare-payload landing stays single-file and the stamped envelope is written to the durable store copy only. The driver seats a fix auditor on a runner-backed vendor (`codex`/`cursor`) whenever one is live; on a durable-record session with none live, or when an independent-family vendor is live only as a host seat, `next` refuses `auditor-unseatable` rather than emit the audit order, and the remedy is a fresh session seeded with a runner vendor in `--vendors`, or hand `next`/`submit` for the whole session. A host seat carries no runner record, so certification refuses `unrun-review` for it. |
...
2. **Dispatch exactly one action** (panel, verifiers, synthesis, gap-sweep, audits, scoped-finder, verify, fixer, judgment gate, or stall menu) — one **driver phase**; when that phase is panel, verifiers, or audits, fan its members out per `reference/round-driver.md` § Batch concurrency — every other phase is fulfilled once. Round 1 = full `reviewer-deep` panel; rounds 2+ = delta rounds (fix audits + scoped finder) unless the driver schedules a full panel (#174 re-arm or unknown surface → run-everything). Degraded/single-vendor: same driver, same journal, `independence: "degraded"` stamps — stay on the path. **Fold path:** roster-bearing `dispatch-*` phases may use the durable-record path (`record-result` / `record-missing` + `advance` per `reference/round-driver.md` § Durable-record path) **or** the hand path (compile artifact + `submit`); owner gates (step 4) fold by hand `submit` on a hand-path session and by `advance --owner-artifact` on an advance-path session. The two fold paths are mutually exclusive per session (see §3 above).
3. **Durable-record path:** write each seat's landing (orchestrator writes engine-seat landings from folded `dispatch-review` stdout), then `record-result` / `record-missing` with echoed `round` and `phase` from step 1 — every runner-dispatched seat's `record-result` carries `--evidence-run-dir <the completed dispatch-review run directory>`; omitting it leaves the seat's execution evidence absent, which withholds the certificate — then `advance` — no hand `submit`. **Hand path:** write the artifact JSON, then `submit` with echoed `phase`, `attempt`, and `expectedStateHash`.
4. <!-- decision-point: id=review-present-judgment mode=gate kind=owner-gate default="fix-as-suggested" carrier=review-code-meta -->
   On `present-judgment` (a tradeoff/product-choice blocker — an **intervention gate, not a terminal**), present each `payload.findings[]` with its `dispositions` (`fix-as-suggested`, `fix-with-guidance`, `skip`) and submit `{dispositions: [{id, disposition, guidance?, reason?}, ...]}` — `skip` needs a citable `reason`; a genuine owner ruling **MUST** also carry a `_provenance` block (shape: `reference/round-driver.md` Owner gates) — without one the fold is `owner-unattributed`, which is correct when no owner ruled and must never be faked; the driver folds fixes back into the fix leg and rides skips on the exit disclosure (fail-closed: a missing/unknown disposition folds as `fix-as-suggested`). When gate policy has not pre-authorized a resolution, the choice is **written down** in the durable session record and the run **hands back** — a missing or unknown disposition still folds as `fix-as-suggested`, never a silent skip. Follow-up: `/superheroes:configure`.
   <!-- /decision-point: id=review-present-judgment -->
   <!-- decision-point: id=review-present-stall-menu mode=gate kind=owner-gate default="park" carrier=review-code-meta -->
   On `present-stall-menu` (the audit-stall **owner gate** — reached only after one invisible self-recovery, **not** a terminal), present `payload.choices` — the current stall vocabulary (`one-more-round`, `accept-the-disclosed-risk`, `hold`); `accept-the-disclosed-risk` only when `payload.acceptRiskEligible` (a stalled audit target that is CONFIRMED with evidence); `one-more-round` only when it appears in `payload.choices` (once per session). Submit `{choice: "<stall choice>"}`; a genuine owner ruling **MUST** also carry a `_provenance` block (shape: `reference/round-driver.md` Owner gates) — without one the fold is `owner-unattributed`, which is correct when no owner ruled and must never be faked. `hold` → terminal `held`. `one-more-round` → not a terminal; re-enters `dispatch-fixer` → `dispatch-audits`. Never judge the dispute yourself. When gate policy has not pre-authorized a resolution, `advance` **parks** (`advance-stall-park`) — the stall choice is **written down** and the run **hands back**. Follow-up: `/superheroes:configure`.
   <!-- /decision-point: id=review-present-stall-menu -->
...
The fixer subagent prompt template is in `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}/rubric/orders/dispatch-fixer.md` — the driver renders it per `dispatch-fixer`; the fenced block in `skills/review-code/reference/auto-fix-loop.md` is illustrative only. Embed `REPO_ROOT` (absolute) into the fixer prompt's `## Input` block. On `dispatch-fixer`, submit `headDiff` and `changedSubjects` derived from git via the **guarded** per-round command in Setup (`git … diff --no-color --no-ext-diff --no-textconv "$BASE_REF"...HEAD` (its config pins included) against the pinned base commit, with its failed-diff and empty-diff halts), never the fixer's self-report.
...
**Then, after the summary**, run the non-blocking end-of-run steps from `## Learning Loop & Staleness Nudge`: (1) the **staleness nudge** (print the doctor `message` only when non-null and `nudge_acked` is false). The **learning-loop proposal** and the **provisional-profile confirmation** are not run — the learning loop learns from decisions a human made, and a gate nobody answered produced none.
...
After the single pass, write the presentation to `$SESSION_DIR/round-1/presentation.md` and print a terminal report. No commits. (A profile with `mode: review-only` makes the default path degrade into exactly this presentation.)

**If context was compacted between dispatch and presentation**, re-read `$SESSION_DIR/round-1/compiled.json` and `$SESSION_DIR/meta.json` to restore state (§ Session Directory). The skill is resumable from disk.

**Write the presentation artifact — the only path.** Follow `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}/skills/review-code/reference/headless-presentation.md`. Presence is an event, not a state a run can detect — the run never branches on whether a human is present and never opens a question that could block waiting for an answer. Never open `AskUserQuestion` on this path or any review-code path.

**Form the orchestrator POV before writing.** Per the base rubric's "Orchestrator POV", for each Critical/Important finding open the cited file at the cited line (in `$SESSION_DIR/repo/` for the PR path, working tree otherwise) and form a **Fix / Skip / Defer + one-sentence rationale + High/Low confidence** take. This is the coordinator's own judgment from a small targeted read — not a re-review. For batched Minor/Nit, derive the POV from the finding text (read the file only if the text is insufficient).

**Apply the review gate.** Partition findings by POV: `auto-include` = `recommendation == Fix` (these land under **Approved — POV `Fix`** in the artifact); `ask-set` = `recommendation` is `Skip` or `Defer` (these land under **Undecided — needs a human call**). The `ask-set` is neither approved nor dropped — listed in full for the human who reads the artifact.

Write `$SESSION_DIR/round-1/presentation.md` per `headless-presentation.md` § The artifact:

1. Verdict banner and one-line summary (including any base-fetch degradation line).
2. **Approved — POV `Fix`.** The `auto-include` set. Per finding: severity tag, `file:line`, title, body, and the POV line.
3. **Undecided — needs a human call.** The `ask-set`, listed in full:
   - **Critical and Important findings (ask-set) — individually.** For each: severity tag, dimension(s), `file:line`, the finding text, the suggested fix, and — on its own line — the **POV** (e.g. `→ POV: Skip (Low confidence) — correct in theory but this path is never hit concurrently under the profile's threat model`).
   - **Minor and Nit findings (ask-set) — batched for readability.** Group in batches of 4. For each finding: severity, `file:line`, a 2-3 sentence summary, and a compact POV tag (e.g. `POV: Skip (Low)`).
4. A count summary that states the two sets separately (e.g. `"3 Critical, 5 Important approved; 2 Important, 4 Minor undecided"`).
