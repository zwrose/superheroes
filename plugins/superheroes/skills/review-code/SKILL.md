...
| Landing — **host** seat (`claude` native subagent) | `$SESSION_DIR/round-<N>/landing/<phase>/<skey>.a<K>.payload.json` — write **only** the payload; the driver wraps with the emitted stub at ingest. At state v5 the ingested envelope is `seat-result/2`; when execution evidence is stamped the bare-payload landing stays single-file and the stamped envelope is written to the durable store copy only. On `dispatch-audits` a host seat can never carry the runner record its landing needs, so on a durable-record session (state v5, folding through `advance`) the driver seats auditors only on a runner-backed vendor (`codex`/`cursor`). When none is live, the audit order refuses at emission with `order-render-refused:<skey>:auditor-no-runner-record:<vendor>`, before any dispatch. Recover with a fresh session whose `--vendors` includes `codex` or `cursor`. A hand-`submit` session keeps its old seating and does not apply this gate. A host seat on a finder phase (panel, scoped finder, gap sweep) with no execution evidence sits out of the certified panel and is named on the receipt; a host synthesis or verifier seat never does (`reference/certification-surface.md`). |
