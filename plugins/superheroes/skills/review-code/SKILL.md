...
| Landing — **host** seat (`claude` native subagent) | `$SESSION_DIR/round-<N>/landing/<phase>/<skey>.a<K>.payload.json` — write **only** the payload; the driver wraps with the emitted stub at ingest. At state v5 the ingested envelope is `seat-result/2`; when execution evidence is stamped the bare-payload landing stays single-file and the stamped envelope is written to the durable store copy only. On `dispatch-audits` at state v5 a bare-payload host-seat landing carries no runner record and is refused `provenance-underivable` on the durable-record path — on a session already on the durable-record path the driver refuses to emit an auditor order for a host seat (`order-render-refused`, detail `discharge-phase-native-seat`), so a session whose selected auditor is a `claude` seat (including the single-vendor `claude` configuration, where the degraded auditor is still `claude`) must fold by hand `submit` for the whole session; seat auditors on a runner-backed vendor (`codex`/`cursor`) when one is live. Proving a native host seat ran is C13 layer 4 scope. |
