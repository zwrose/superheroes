...
```bash
python3 -B "$ROOT_DIR/lib/round_driver.py" review-diff --base "$BASE_REF" --repo-root "$REPO_ROOT" > "$SESSION_DIR/round-<round>/diff.txt.tmp" && mv "$SESSION_DIR/round-<round>/diff.txt.tmp" "$SESSION_DIR/round-<round>/diff.txt" || { rm -f "$SESSION_DIR/round-<round>/diff.txt.tmp"; echo "review-code: review diff against $BASE_REF FAILED — refusing to review the artifact it left behind (#637)" >&2; exit 1; }
