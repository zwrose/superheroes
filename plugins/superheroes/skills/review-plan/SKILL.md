...
Capture the JSON in `DOCTOR_JSON`. On `readable: false`, tell the user "profile unreadable — re-run `/superheroes:configure`" and **continue** (do not crash, do not block). Otherwise retain `message`, `signal_hash`, and `nudge_acked` for the **end-of-run staleness nudge** (see §5's terminal summary). Do NOT act on `drift` here — it is informational only.
...
The common-mistake table for review-plan is in `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}/skills/review-plan/reference/plan-detail.md` — read it before finalizing findings.
