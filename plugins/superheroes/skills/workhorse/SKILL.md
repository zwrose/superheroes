...
reader rather than implying something is running. Run the **calibrated local gates** (the project's verify command, whatever the calibration names) and **watch CI** — CI is the suite's receipt, in the shape `rubric/test-receipt-evidence.md` names: a **successful** conclusion of **the workflow that runs the suite** (this repository's `CI`; never a title or lint workflow), selected by **workflow name and exact head sha**, with evidence it runs the claimed tests. **Beyond the calibrated verify command, the full suite runs locally at most once per build, at the final head, and not at all when that suite workflow already has a successful run on that head**: a second local run proves nothing CI does not, and each one is ten minutes the lane spends waiting (seven lanes measured 2026-09-19). The calibrated verify command runs every time the charter says, even where the calibration makes it the suite itself; the bite-proof red/green runs are never what this skips. **A local full-suite run starts only on a clean, settled tree —
...
Where the probe runs — and why a tree with live readers is the wrong place — is stated once in
...
| "The implementer says tests pass" | Re-run the calibrated verify command and the bite-proof runs yourself and read the raw output; the suite's receipt is the suite workflow's success, by workflow name, on the head sha. Verification authority never delegates. |
...
| "The dead session's PR body says the tests passed — that's my receipt" | It is an inherited claim, not a receipt. Re-run the calibrated verify yourself (the suite's receipt is the suite workflow's success, by workflow name, on the head sha), and sweep its worktrees for work it never pushed before you build on the pushed tip. |
