...
reader rather than implying something is running. Run the **calibrated local gates** (the project's verify command: the validators and the touched-tests selection) and **watch CI** — CI is the suite's receipt. **The full suite runs locally at most once per build, at the final head, and not at all when CI is already green on that head**: a second local run proves nothing CI does not, and each one is ten minutes the lane spends waiting (seven lanes measured 2026-09-19). **A full-gate run starts only on a clean, settled tree —
