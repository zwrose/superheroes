...
reader rather than implying something is running. Run the **calibrated local gates** (the project's verify command: the validators and the touched-tests selection) and **watch CI** — CI is the suite's receipt, in the shape `rubric/test-receipt-evidence.md` names: a **successful** conclusion of the named workflow on the **exact head sha**. **The full suite runs locally at most once per build, at the final head, and not at all when that named workflow already has a successful run on that head**: a second local run proves nothing CI does not, and each one is ten minutes the lane spends waiting (seven lanes measured 2026-09-19). **A local full-suite run starts only on a clean, settled tree —
