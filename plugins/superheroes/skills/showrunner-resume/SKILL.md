...
| **2. re-arm** | **All** of: the recorded leader process is **positively live** (the double-confirmed probe in Step 3); the lane's heartbeat sweep does **not** classify the lane **terminal** (`parked` or `handback`); the lane's session transcript **resolved** (Step 3, source 6: exactly one file found by the identity rule, not dated into the future) **and** written within `LIVENESS_QUIET_WINDOW_SECONDS` (`lib/heartbeat.py`, bound in `lib/wave_watch.py`) of the read. **A transcript alone never re-arms.** | Nothing is relaunched and no outcome recorded; the lane's batch is a candidate for Step 6 arming. |
...
Lanes: vet 1 (#220 — PR exists, non-draft, durable review receipt, remote head = receipt commit, CI success on that sha by workflow name); re-arm 1 (#221 — leader positively live, heartbeat nonterminal, transcript resolved and written within `LIVENESS_QUIET_WINDOW_SECONDS`); adopt 0; parked 0; unresolved 0.
...
Lanes: vet 0; re-arm 1 (#306 — leader positively live, heartbeat nonterminal, transcript resolved and written within `LIVENESS_QUIET_WINDOW_SECONDS`); adopt 0; parked 0; unresolved 1 (#305 — row 5: remote head read failed).
