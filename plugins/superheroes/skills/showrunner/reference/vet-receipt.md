...
| the build record records a **full-lane** pre-handback review | the **certified-loop check** — per `rubric/review-discipline.md` driver-mandate flip: **before** the milestone *Review panels can't silently degrade* closes, the driver's own round receipt, or a skip disclosure citing an **open** `driver-blocker` issue by number. A citation that is absent, or that names a closed issue, is a finding; **after** that close, the driver's own round receipt, or an explicit **parked** outcome — a cited skip is no longer a pass |
