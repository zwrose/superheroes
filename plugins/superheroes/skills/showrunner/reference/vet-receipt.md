...
| the PR carries a **build record** (a full or light build) | the **size-tripwire check** — the build record's size tripwire row against the non-test count measured at each commit in the PR's history (the first commit past 2×, if any, is the crossing — a final count back under 2× does not make it `not crossed`); a crossed row cites the ruling or park recorded on the issue, except the `disclosed to the owner in session` form, which stands on the row for a build the owner drove with no advisor named; a missing row is a finding. The step and the row's forms live in the workhorse charter (§4 size step, §11 build record) |
