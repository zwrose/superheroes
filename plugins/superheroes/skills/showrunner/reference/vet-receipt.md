...
| the `vet-checks` verb reports `declared: true`, or a non-null `reason` other than none | one receipt line per declared check, and a finding per malformed entry or unreadable calibration — see [Project vet checks](#project-vet-checks) |
...
was broken. Malformed entries report as `{"index", "field", "reason"}`; only well-formed entries
appear in `checks` — never a partial list.
...
both lists are empty. `vet-checks-malformed` is reported through `malformed` with `checks` empty.
