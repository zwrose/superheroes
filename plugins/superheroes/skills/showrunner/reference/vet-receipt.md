...
| the PR carries a **build record** (a full or light build) | the **size-tripwire check** — the build record's size tripwire row against the diff's measured non-test count; a crossed row cites the ruling or park recorded on the issue; a missing row is a finding. The step and the row's forms live in the workhorse charter (§4 size step, §11 build record) |
