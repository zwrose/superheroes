...
| the PR carries a **build record** (a full or light build) | the **size-tripwire check** — the build record's size tripwire row against the non-test count at each commit in the PR's history and the build's starting estimate, both as `rubric/review-discipline.md` § Size defines them (the first commit past the tripwire is the crossing, even when a later count falls back under it); a crossed row cites the ruling or park recorded on the issue, except the `disclosed to the owner in session` form for a build the owner drove with no advisor named; a missing row is a finding. The row's forms live in the workhorse charter §11 |
