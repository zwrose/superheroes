...
7. **The door helper.** A session reaches for `lib/front_door.py` and its `front_door.grade` entry
   ...
   `evidence-argued`, `p0-band-excluded`, `p0-definition-unstamped`, and `profile-absent`, among
   others; **`p0-definition-unstamped` refuses a P0 claim when the project's optional P0-definition
   configuration item is unstamped — the door does not fall open to an ungoverned P0**; an unstamped
   ladder **queues** a P2 claim rather than refusing it. The helper's own evidence tokens — `field`, `lab`,
...
3. **Every vet receipt states the misses-log appends it made, or `None`,** following that receipt's
...
1. **A declined item becomes a registry line with a named trigger**. The trigger is a concrete
   ...
   grid wherever it now lands**, any tier up to P0. When the owner has named a risk and the
   recommendation is to decline, a declined owner-named risk ships with a mechanical tripwire set at
   a threshold no looser than twice the worst observed instance, because a decline without a
   tripwire is a bet nobody is watching. Declined is a pre-evidence state, not a graveyard.
...
craft call. A **venue-2 scope change** is machinery whose resolved venue changes committed scope
beyond continue-in-lane; the door yields an owner call. A **product decline** trades away something a
product reading could want; product work never took the bar, and the door yields an owner call. An
item whose **door grading is uncertain** — band, evidence tier, or resulting tier not yet settled —
has not completed the door's decision and yields an owner call until grading is clear. A **ride-along**
passed only at continuation cost; the door yields eligibility for the continue and fold venues only,
