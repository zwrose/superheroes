...
key `tasks.md` (a `$SESSION_DIR/...` session path never matches a prior run's identity) — then
...
import os as _os
for x in findings:
    if isinstance(x, dict) and isinstance(x.get('file'), str) and _os.path.basename(x['file']) == 'tasks.md':
        x['file'] = 'tasks.md'   # stable ledger key — session paths never match across runs
