...
- The launch prompt the advisor drafts is the workhorse command plus the issue pointer only;
  anything the build must follow sits in the issue body, not in the prompt. The standing-rulings
  block the launcher itself appends from its doctrine file is allowed; hand-written extra
