...
| P3 | Profile present but the **staleness check itself can't read it** (`readable: false`) | Tell the user "profile unreadable — re-run `/superheroes:review-init`" and **continue** (do not crash, do not block). `repo_doctor.py` fails soft (always exit 0). | R-CODE §1 Setup; R-PLAN §1; R-DEBT §1; DOCTOR module docstring ("FAIL SOFT … NEVER crash") | yes | already correct |
