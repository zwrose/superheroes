...
| `assumption-violation` | An unstated assumption that, if false, breaks the design. Plan-time by default — and, in code mode, the **new entry point into an unchanged surface** case below, whose assumption is that the pre-existing surface handles what the new entry point hands it |
...
- **`assumption-violation`** (plan-time, plus the code-mode entry-point case below) — an assumption the plan relies on but never states (single-writer, ordering, idempotency of a callee, dataset size). State the assumption, the realistic scenario where it is false, and what breaks. **Important** by default; **Critical** only when the violated assumption corrupts data with no recovery path.

**Environment-supplied posture — a `fail-direction` sub-lens, in code mode as well as plan mode.** Ask of every changed gate, guard, verification step, or check: *is this green because the code satisfies it, or because **this** environment supplies what it asks for?* An ambient git identity, an allow rule or permission grant this account happens to hold, a harness that pre-satisfies the gate's own precondition, reachable network, a binary already on `PATH` — each makes the gate report pass **without having tested the thing**, which is the ratified fail-direction shape ("a verify step that trusts an ungrounded/echoed answer, a certification that proceeds on missing evidence") arriving by supply rather than by default value. It is a finding when the diff neither names what is being supplied nor demonstrates the check red without it; the guard to propose is the **A/B against the real path** — run it with the supplied thing removed or overridden — or, where that genuinely cannot be constructed here, an explicit statement of which environment does prove it. Three-leg chain: **trigger** is the ordinary event of running where the supply is absent (the CI runner, a fresh clone, another operator's box); **propagation** is that nothing else re-asks the question, because the gate already answered pass; **impact** is the unverified thing shipping behind a green that measured the runner. **Critical** when the unverified thing is what lets unreviewed or uncertified work ship; **Important** otherwise. *Teaching examples:* the **2026-08-09 CI-identity escape** — a dev box silently supplied `user.email`, so the local run was green **by construction** and unreproducible locally for exactly that reason, while both halves of the stack went red in CI behind a local ready; and the **pre-proven-liveness fixture class**, PR **#714**, where the harness defaulted an engaged liveness result the fixtures leaned on and `4051 passed, 3 skipped` carried a park-to-certify regression through untouched. Where the supplied thing sits in a *test* rather than in the machinery, the finding is `test-reviewer`'s **environment-supplied-posture** smell, not yours — yours is the gate.

**New entry point into an unchanged surface — label it `assumption-violation`.** A diff that adds an **entry point into a pre-existing surface** — a new route, command, flag, menu item, caller, or trigger that reaches code the diff does not touch — carries an unstated assumption: *that surface handles what this entry point hands it, in the state this entry point arrives in.* Nothing in a diff-scoped review tests that assumption, and that is a **structural blind spot of diff-scoped review itself**, not an oversight by any one seat: the entry point is on a `+` line so it is in scope, the surface it reaches is unchanged so nobody reads it, and the composition of the two is what breaks. The obligation the diff owes is **one end-to-end exercise through the new entry point** — driven from the entry point, asserting on what the surface actually produces at the far end, not a unit test of the entry point's own wiring. Flag it when the diff adds such an entry point and shows no such exercise. **Important** by default; **Critical** when the unexercised path is the change's headline behaviour or the only way a user reaches it. This is yours and not `test-reviewer`'s coverage lens because the missing thing is not a test case on a changed unit — it is the untested *composition* of changed and unchanged code. *Teaching example:* the weekly-eats FR-5 escape (a consuming project's requirement, not this repo's `FR-5`) — a new entry point into an existing surface survived the build's own tests, three pilot audits, and five seats across three rounds, and the owner hit it in minutes the first time they drove the real entry point. The pilot-side half of this lens — making the plan drive the new entry point end to end — is **#945**; this is the review-class half.
...
- **Missing test cases** — `test-reviewer`'s. A coverage gap may inform your narrative but is not your finding to raise. **One carve-out, and only one:** the missing **end-to-end exercise through a newly added entry point into an unchanged surface** (see What to Flag) is yours, because the untested thing is the composition of changed and unchanged code rather than a case on a changed unit. Everything else that reads as "add a test" stays `test-reviewer`'s.
