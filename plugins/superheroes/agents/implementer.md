...
- **In-dispatch verification is targeted.** Your in-dispatch verification covers the tests for **the
  ...
  **Targeted is keyed to the changed behaviour and the work-order surface, not to "the files you
  ...
  a full-suite or project-wide gate: precedence rung 5.** The orchestrator re-runs the
  **calibrated verify command** itself and **CI runs the suite on the pushed head**, so nothing is lost. Long in-dispatch output is what makes an external-engine dispatch
...
- **If you could not run it, say so — never narrate a run you did not make.** Precedence rungs 1–7;
  ...
  calibrated verify command, with CI's suite run on the pushed head, is what closes the loop** — your missing receipt does not make the work accepted-as-green;
