...
each `*` becomes one space, runs are not collapsed — so emphasis markers inside a literal's span
can insert extra spaces that break the exact substring match: `only **what they approve**` does
not match the forbidden literal `only what they approve` (a stated limitation, same family as
underscore-emphasis). A lone `*` standing where a space already belongs still matches. **Forbidden-primitive** matching also joins
