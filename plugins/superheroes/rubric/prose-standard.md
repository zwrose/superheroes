...
or review any of those. A reviewer cites this file; a writer works from it. Nothing checks it
...
- Apply the no-op test sentence by sentence: does this change behavior against the default? The
  test is model-relative; settle a disagreement by running the document, not by debate. When a
...
- Tutorial: action, for learning. Rare in this plugin. The learner's success is your job; every
...
- Link with words that say where the link goes. Prefer a sentence of context on the page to a link
  off it.
- Headings carry the point, in sentence case. A task heading is a verb phrase; a concept heading is
...
- One instruction per sentence; one thought per sentence everywhere else. Split an instruction
...
- Prefer a plain verb to an "-ing" form; "-ing" words take too many grammatical jobs.
...
sentence that carries two thoughts; keep the long one that carries one. Have a view where the mode
...
- Bold on every proper noun; a bold label and colon that restate the line ("**Speed:** speed
...
- Rationale stays on counter-default rules: the rule a reader would not guess gets its reason in
  one sentence; the rule every careful engineer already follows gets none.
