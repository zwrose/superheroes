...
#515/#517). Its **code-leg (review-code) dispatch ships under #609** (the seat map itself shipped under
#510); the review-code orchestrator **retains the inline PR-body honesty check as a second
leg** covering `external`-verifiability claims the sanitized-view seat cannot settle — not
an interim stand-in. As a code-leg lens it is not one of review-code's five dispatched
dimensions, and no code-leg drift test should read it as one.
