...
Your groups must account for **every** staged survivor id **exactly once** under a **coverage
...
A submit that carries **no group-list answer at all** is **refused** at submit — the driver
names what is missing; an explicit **`null`** or **empty** group list carries no coverage
obligation and the driver falls open to unmerged survivors; **synthesis drops nothing**
and synthesis failure never aborts the review. `verification.merge_and_rank` applies accepted groups mechanically; merged groups combine bodies and take
