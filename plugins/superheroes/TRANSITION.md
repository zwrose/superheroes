...
A successful launch that ran the dependency gate carries `dependencyGate` on its result — the gate's
applied flag, reason, and when applied the dependency number, head, and verdict.
