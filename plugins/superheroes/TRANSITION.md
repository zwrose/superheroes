...
The launcher's builder argv is `claude --bg --model <tok> [--effort <e>] <prompt>`, built by
`engine_adapter.claude_builder_argv`.

`compose` no longer returns `sessionId`.

The `reserved` record no longer carries `sessionId` for a background launch.

The `started` record's `pid` is the background session's own process, and the record gains
`launchMode: "background"`, `backgroundId`, and `sessionId`.

The launch result gains `sessionId` and `backgroundId`.

New launch refusals:

- `config-dir-unusable:unresolvable`
- `config-dir-unusable:not-a-directory`
...
A failed launch whose stop cannot be confirmed names `orphanedBackgroundId` and a `remedy`.

**`record-outcome` on a background lane stops the session first.** It refuses
`background-stop-unconfirmed` (writing nothing) when the stop cannot be confirmed.

**Older checkouts reading the same ledger:**

- they still fold the new records;
- their watch loop reads a working background lane as live;
- their `record-outcome` refuses `terminal-child-live:<pid>` on a finished background lane until you
  run `claude stop <backgroundId>` under the lane's `CLAUDE_CONFIG_DIR`;
- their watch loop cannot use the transcript to quiet a stale-heartbeat alert on a background lane,
  so it may wake you more, never less.

New verb: `launcher.py canary --repo-root R --launch-id L` reports whether the lane's session
transcript shows at least one tool call (`canary-not-engaged` / `canary-transcript-unresolved`).

**Prerequisite:** `claude --bg` refuses to start in a directory whose workspace trust has not been
accepted (`Workspace not trusted …`), and the launch refuses `background-launch-failed` with that
line as evidence. Open `claude` once in the build-worktrees root and accept the trust prompt.

A dispatched `claude_cli` read with an invalid argument list now returns exit `127` without spawning.
