...
def read_membership(
    *,
    ...
    if expect_stack is not None and not _is_int(expect_stack):
        return _refusal(
            REASON_BAD_ARGUMENT,
            "expect_stack must be a positive integer",
            repo=repo,
            pr=pr,
        )
    ...
    while True:
        ...
        if proc.returncode != 0:
            return _refusal(
                ...
                _stderr(proc) or "gh api graphql failed",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        try:
            ...
            return _refusal(
                REASON_STACK_UNREADABLE,
                "gh api graphql returned output that is not JSON",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if not isinstance(payload, dict):
            return _refusal(
                REASON_STACK_UNREADABLE,
                "gh api graphql returned JSON that is not an object",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if not isinstance(errors, list):
            return _refusal(
                REASON_STACK_UNREADABLE,
                "gh api graphql errors is not a list",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if not isinstance(data, dict) or not isinstance(repository, dict):
            return _refusal(
                REASON_STACK_UNREADABLE,
                "graphql data/repository is null or not an object",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if not isinstance(pull_request, dict):
            return _refusal(
                REASON_STACK_UNREADABLE,
                "graphql pullRequest is null or not an object",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if stack_entry is None:
            return _refusal(
                REASON_NOT_LINKED,
                "pull request is not in a stack",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if (
            ...
            return _refusal(
                REASON_STACK_UNREADABLE,
                "stack field is missing or of the wrong type",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if pages == 1:
            ...
            if not _snapshot_matches(snapshot, pull_request, stack_entry, stack):
                return _refusal(
                    ...
                    "later page disagrees with page-one snapshot",
                    repo=repo,
                    pr=pr,
                    pages=pages,
                )
        ...
        for node in nodes:
            ...
            if len(collected) + 1 > stack_size:
                return _refusal(
                    REASON_STACK_UNREADABLE,
                    "collected member count would exceed stack size",
                    repo=repo,
                    pr=pr,
                    pages=pages,
                )
        ...
        if not isinstance(has_next_page, bool):
            return _refusal(
                REASON_STACK_UNREADABLE,
                "pageInfo hasNextPage is missing or not a boolean",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if added == 0:
            return _refusal(
                REASON_STACK_UNREADABLE,
                "page added zero nodes while hasNextPage is true",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if not isinstance(end_cursor, str):
            return _refusal(
                REASON_STACK_UNREADABLE,
                "pageInfo endCursor is not a string",
                repo=repo,
                pr=pr,
                pages=pages,
            )
        ...
        if end_cursor in used_cursors:
            return _refusal(
                REASON_STACK_UNREADABLE,
                "endCursor repeats a cursor already used",
                repo=repo,
                pr=pr,
                pages=pages,
            )
    ...
    if expect_stack is not None and expect_stack != snapshot["stack_number"]:
        return _refusal(
            REASON_ORDER_MISMATCH,
            "expect_stack does not equal stack number",
            repo=repo,
            pr=pr,
            pages=pages,
        )
    ...
    if len(collected) != stack_size:
        return _refusal(
            REASON_ORDER_MISMATCH,
            "collected member count does not equal stack size",
            repo=repo,
            pr=pr,
            pages=pages,
        )
    ...
    if positions != expected_positions:
        return _refusal(
            REASON_ORDER_MISMATCH,
            "member positions are not exactly 1..size",
            repo=repo,
            pr=pr,
            pages=pages,
        )
