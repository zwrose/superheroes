...
_BYTE_PIN_LINES = frozenset({
    ...
    "`skip`). Submit `{dispositions: [{id, disposition, guidance?, reason?, followUp?}, ...]}` — `skip` needs a "
    "citable `reason` and may carry an optional `followUp` object. Fixes fold into the round's fix batch and the loop proceeds into the fix leg; "
    ...
    "Submit `{choice, followUp?}` — `accept-the-disclosed-risk` may carry an optional `followUp` object. **`hold`** → terminal `held`, certification withheld (absorbs the retired "
