...
def test_C13_two_round_moving_head_no_stale_verify_then_backfill(tmp_path):
    ...
    with open(os.path.join(fold_session, RR.META_FILE), "w", encoding="utf-8") as fh:
        json.dump(meta, fh)
    # The submit path resolves the verified head before the fold; the fold records what it is given.
    verified_head, head_err = RD._resolve_fix_fold_head_sha(fold_session, state)
    assert head_err is None, head_err
    assert verified_head == head2
    RD._fold_verify(state, state["config"], {"result": "pass"}, verified_head=verified_head)
    entry = _ledger_by_key(state)[key]
