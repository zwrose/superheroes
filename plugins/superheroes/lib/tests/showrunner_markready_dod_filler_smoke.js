...
;(async () => {
  ...
  assert.deepStrictEqual(labels, ['mark PR ready', 'wait for CI to settle', 'fill-dod', 'splice DoD dispositions', 'mark PR ready'])
  ...
  assert.deepStrictEqual(labels, ['mark PR ready', 'wait for CI to settle', 'fill-dod', 'splice DoD dispositions'])

  // proposal returns no rows -> no splice, no re-decide.
  ;({ sr, labels } = run({ gateSeq: [DOD_PARK], propose: { ok: true, rows: [] }, splice: { ok: true } }))
  out = await sr.markReadyPhase('wi-dod')
  assert.strictEqual(out.phaseResult.confidence, 'low')
  assert.deepStrictEqual(labels, ['mark PR ready', 'wait for CI to settle', 'fill-dod'])
  ...
  out = await sr.markReadyPhase('wi-dod')
  assert.strictEqual(out.phaseResult.confidence, 'low')
  assert.deepStrictEqual(labels, ['mark PR ready', 'wait for CI to settle', 'fill-dod'], "ok:'false' never splices")
