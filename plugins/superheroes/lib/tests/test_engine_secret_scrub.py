...
def test_scrub_mapping_scrubs_nested_keys_structural_values_and_other_fields():
    ...
    out = engine_adapter._scrub_mapping(salvage_in)
    blob = json.dumps(out)
...
def test_scrub_mapping_three_secret_keys_all_values_survive():
    # axis: scrubbed-key collision preserves every entry — not injective key scrub.
...
def test_scrub_mapping_nested_secret_key_collision_preserves_entries():
    # axis: scrubbed-key collision preserves every entry — not injective key scrub.
    nested = {_ANT_KEY_A: "inner1", _ANT_KEY_B: "inner2"}
    out = engine_adapter._scrub_mapping({"wrapper": nested})
    inner = out["wrapper"]
    assert set(inner.values()) == {"inner1", "inner2"}
    assert len(inner) == 2


def test_scrub_mapping_no_collision_keys_byte_identical():
    # axis: scrubbed-key collision preserves every entry — not injective key scrub.
...
def test_scrub_mapping_literal_redacted_key_survives_collision():
    # axis: scrubbed-key collision preserves every entry — not injective key scrub.
