...
def test_unified_layer_with_schema_in_body_stays_readable(tmp_path, capsys):
    """Unified layer whose focus hints mention schema: must not fall into legacy provenance parse."""
    root, env = make_root(tmp_path, src_dirs=["src"], deps=["a"])
    layer = tmp_path / "review-crew.md"
    layer.write_text(
        "<!-- review-crew: schemaVersion=1 status=confirmed created=2026-01-01 "
        "updated=2026-01-01 rubric-version=2 nudge-ack={} -->\n\n"
        "## Focus hints\n- test: validate against schema: draft-07 for API payloads\n"
    )
    rc, res = run(str(layer), "review-crew@0.1.0", 2, root, env, capsys)
    assert rc == 0
    assert res["readable"] is True
    assert res["ok"] is True


def test_unified_layer_rubric_version_drift(tmp_path, capsys):
    """Layer provenance rubric-version enables rubric drift on unified layouts."""
    root, env = make_root(tmp_path, src_dirs=["src"], deps=["a"])
    layer = tmp_path / "review-crew.md"
    layer.write_text(
        "<!-- review-crew: schemaVersion=1 status=confirmed created=2026-01-01 "
        "updated=2026-01-01 rubric-version=1 nudge-ack={} -->\n\n## Focus hints\n- code: x\n"
    )
    rc, res = run(str(layer), "review-crew@0.1.0", 3, root, env, capsys)
    assert rc == 0
    assert any("rubric-version" in d for d in res["drift"])


def test_unified_layer_bare_key_nudge_ack(tmp_path, capsys):
    """Bare-key nudge-ack form ({<hash>: true}) is tolerated on unified layers."""
    root, env = make_root(tmp_path, src_dirs=["src"], deps=["a"])
    layer = tmp_path / "review-crew.md"
    layer.write_text(
        "<!-- review-crew: schemaVersion=1 status=confirmed created=2026-01-01 "
    ...
    layer.write_text(
        "<!-- review-crew: schemaVersion=1 status=confirmed created=2026-01-01 "
        "updated=2026-01-01 rubric-version=1 nudge-ack={%s: true} -->\n\n"
        "## Focus hints\n- code: x\n" % h
    )
