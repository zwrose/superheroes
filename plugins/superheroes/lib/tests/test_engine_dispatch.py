...
def test_run_engine_files_caps_stdout(tmp_path, monkeypatch):
    ...
    tail_text = ED._read_capped_text(stdout_path, stream=ED.CAP_STREAM_STDOUT)
...
def test_run_engine_files_caps_only_after_terminate_on_timeout(tmp_path, monkeypatch):
    ...
    def obs_cap(path, max_bytes, stream):
        if path == stdout_path:
        ...
        return real_cap(path, max_bytes, stream)
...
def test_fold_stream_activity_mtime_wins_when_later_than_tick(tmp_path):
    """axis: post-reap fold credits file mtime when it beats the last in-band tick."""
    stdout_path = str(tmp_path / "stdout")
    stderr_path = str(tmp_path / "stderr")
    with open(stdout_path, "w", encoding="utf-8") as fh:
        fh.write("growth")
    prev_stdout = 0
    tick_moment = 1_000_000.0
    expected_mtime = 1_000_500.0
    os.utime(stdout_path, (expected_mtime, expected_mtime))
    stored_mtime = os.path.getmtime(stdout_path)
    last_activity_at, silence, activity_stream = ED._fold_stream_activity(
        stdout_path, stderr_path, prev_stdout, 0, tick_moment, None,
    )
    assert activity_stream == "stdout"
    assert last_activity_at == stored_mtime
    assert silence is not None


def test_fold_stream_activity_tick_wins_when_later_than_mtime(tmp_path):
    """axis: post-reap fold credits in-band tick when it beats file mtime."""
    stdout_path = str(tmp_path / "stdout")
    stderr_path = str(tmp_path / "stderr")
    with open(stdout_path, "w", encoding="utf-8") as fh:
        fh.write("growth")
    prev_stdout = 0
    tick_moment = 1_000_500.0
    earlier_mtime = 1_000_000.0
    os.utime(stdout_path, (earlier_mtime, earlier_mtime))
    last_activity_at, silence, activity_stream = ED._fold_stream_activity(
        stdout_path, stderr_path, prev_stdout, 0, tick_moment, None,
    )
    assert activity_stream == "stdout"
...
def test_mode_invalid_refused_at_library_boundary(tmp_path):
    ...
    assert res["rejectedMode"] == "'bogus'"


class _ModeEqRaises:
    def __eq__(self, other):
        raise RuntimeError("mode-eq-boom")
...
def test_mode_invalid_non_string_with_eq_raises_returns_structured_refusal(tmp_path):
    ...
    assert res["terminal"] is True
    assert res["mode"] == _SV_MOD.MODE_REVIEW
    assert "ModeEqRaises" in res["rejectedMode"]
...
def test_continuation_explicit_brief_check_with_diff_base_still_refused(tmp_path):
    run_dir = str(tmp_path / "run")
    repo_root, _ = _manual_open_review_run_with_mode(tmp_path, run_dir, mode="brief-check")
    res = ED.dispatch_review(
        seat=_brief_check_codex_seat(),
...
def test_dispatch_review_brief_check_end_to_end(tmp_path):
    ...
    assert res["sanitizedView"]["diffBytes"] is None
    assert res["sanitizedView"]["diffWithheldCount"] is None
    # diff_base stays None because brief-check + explicit diff_base refuses at engine_dispatch.py:2725
    assert build_view.captured["kwargs"][0]["diff_base"] is None
    prompt_text = fake.calls[0]["prompt_bytes"].decode("utf-8")
    assert "no diff for this review" in prompt_text
    assert "build brief written before the code exists" in prompt_text
...
def test_brief_check_notice_absent_from_default_review_fed_prompt(tmp_path):
    ...
    prompt_text = fake.calls[0]["prompt_bytes"].decode("utf-8")
    assert "no diff for this review" not in prompt_text
...
@pytest.mark.parametrize("label,kwargs,setup,expected_mode,expected_detail,expected_ok,expected_terminal", [
...
def test_stdout_cap_truncation_marker_in_file_tail(tmp_path):
    ...
    ED._cap_file_tail(str(path), ED.MAX_STDOUT_CAPTURE, ED.CAP_STREAM_STDOUT)
...
def test_stdout_cap_truncation_marker_in_read_capped_text(tmp_path):
    ...
    text = ED._read_capped_text(str(path), stream=ED.CAP_STREAM_STDOUT)
    assert ED.STDOUT_TRUNCATION_MARKER_PREFIX in text
    assert ED._stdout_capture_truncated(text)
...
def test_straggler_write_during_cap_window_grades_truncated(tmp_path, monkeypatch):
    ...
    def straggler_cap(path, max_bytes, stream):
        if path == stdout_path:
            with open(path, "ab") as fh:
...
def test_stdout_capped_forfeit_disclosure_attempt1_names_attempt_and_refused_retry(tmp_path):
    ...
    terminal = ED._stdout_capped_forfeit(
        "cursor", 9_000_000, run_dir_real=str(tmp_path), attempts=1)
    disclosure = terminal["disclosure"]
    assert "attempt 1" in disclosure
    assert "retry was refused" in disclosure
...
def test_stdout_capped_forfeit_disclosure_attempt2_names_attempt_not_retry_refused(tmp_path):
    """axis: disclosure names attempt 2 and cites exhausted budget, not a refused retry."""
    terminal = ED._stdout_capped_forfeit(
        "cursor", 9_000_000, run_dir_real=str(tmp_path), attempts=2)
    disclosure = terminal["disclosure"]
...
def test_stdout_marker_overruled_journal_on_authoritative_suppression(tmp_path):
    ...
    assert rec["stampedBytes"] == stamped
    assert rec["markerBytes"] == 9_000_000


def _oversized_digit_forged_head_marker_capture():
    marker = (
        ED.STDOUT_TRUNCATION_MARKER_PREFIX
        + ("9" * 5000)
...
def test_bounded_stdout_cap_file_tail_never_exceeds_budget(tmp_path, monkeypatch):
    ...
    ED._cap_file_tail(str(path), cap, ED.CAP_STREAM_STDOUT)
...
def test_read_capped_text_memory_error_degrades_like_oserror(tmp_path, monkeypatch):
    ...
    assert ED._read_capped_text(str(path), stream=ED.CAP_STREAM_STDOUT) == ""
...
def test_stderr_cap_file_tail_carries_stderr_marker_not_stdout(tmp_path, monkeypatch):
    """axis: truncated stderr file capture carries stderr marker, never stdout marker."""
    run_dir = str(tmp_path / "run")
    os.makedirs(run_dir)
...
def test_stderr_drain_in_memory_carries_stderr_marker_not_stdout(tmp_path, monkeypatch):
    """axis: in-memory stderr drain writes stderr marker, never stdout marker."""
    monkeypatch.setattr(ED, "MAX_STDERR_CAPTURE", 512)
    script = "import sys; sys.stderr.write('e' * 2000); sys.stdout.write('ok\\n')"
    _stdout, _timed_out, _rc, stderr_tail = ED._run_engine(
    ...
    assert "<<<SUPERHEROES-STDERR-TRUNCATED:" in stderr_tail
    assert ED.STDOUT_TRUNCATION_MARKER_PREFIX not in stderr_tail
    assert not ED._stdout_capture_truncated(stderr_tail)


def test_bytes_with_stdout_cap_clamps_marker_to_budget():
    """axis: cap arithmetic never returns more than max_bytes when marker exceeds budget."""
    max_bytes = 10
    data = b"x" * 100
...
def test_cap_arithmetic_single_home_byte_identical(tmp_path):
    """axis: file and memory entry points produce byte-identical capped output."""
    cap = 4096
    over = cap + 2048
    data = b"z" * over
    ...
    with open(path, "rb") as fh:
        file_capped, file_truncated, _observed = ED._bounded_stdout_cap_from_file(
            fh, cap, ED.CAP_STREAM_STDOUT,
        )
    assert mem_truncated == file_truncated
    ...
    with open(small_path, "rb") as fh:
        file_small, file_small_trunc, _obs = ED._bounded_stdout_cap_from_file(
            fh, small_cap, ED.CAP_STREAM_STDOUT,
        )
    assert mem_small_trunc == file_small_trunc
    assert mem_small == file_small
    assert len(mem_small) <= small_cap
...
def _line_porcelain_to_z(text):
    """Convert line-oriented porcelain fixture text to ``-z`` NUL-delimited form."""
    chunks = []
    for line in text.splitlines():
        if not line.strip():
            continue
        if len(line) >= 3 and line[0] in "RC" and " -> " in line:
            status = line[:3]
            new_path, old_path = line[3:].strip().split(" -> ", 1)
            chunks.append(status + new_path.strip())
...
def test_worktree_dirt_exclusion_union_baseline_and_verdict_roots(tmp_path, monkeypatch):
    ...
    call["n"] = 0
    sib_real_verdict = {"path": None}

    def foreign_verdict_only(cwd_real, timeout=None):
        call["n"] += 1
        if call["n"] == 1:
            return set()
        return {sib_real_verdict["path"]}

    monkeypatch.setattr(ED, "_foreign_leased_worktree_roots", foreign_verdict_only)
    baseline = ED._worktree_baseline(cwd, timeout=5)
    assert baseline is not None
    sib_verdict = _nested_sibling_worktree(main, cwd, name="verdict-only")
    sib_real_verdict["path"] = os.path.realpath(sib_verdict)
    with open(os.path.join(sib_verdict, "sibling.txt"), "w", encoding="utf-8") as fh:
        fh.write("peer\n")
    assert ED._worktree_dirt_verdict(baseline, cwd, timeout=5) is False
...
# --- WO-D (#1122): false-clean path fixes ------------------------------------


def test_worktree_dirt_ancestor_exclusion_root_not_admitted(tmp_path):
    main = str(tmp_path / "main")
    _git_init(main)
    nested = os.path.join(main, ".claude", "worktrees", "build")
    os.makedirs(os.path.dirname(nested), exist_ok=True)
    subprocess.run(["git", "-C", main, "worktree", "add", "-q", nested], check=True)
    cwd = os.path.realpath(nested)
    main_real = os.path.realpath(main)
    ancestor_proc = _install_foreign_lease(main)
    try:
        roots = ED._foreign_leased_worktree_roots(cwd, timeout=5)
        assert roots is not None
        assert main_real not in roots
        baseline = ED._worktree_baseline(cwd, timeout=5)
        assert baseline is not None
        with open(os.path.join(cwd, "attempt-left-this.txt"), "w", encoding="utf-8") as fh:
            fh.write("x\n")
        with open(os.path.join(cwd, "and-this.txt"), "w", encoding="utf-8") as fh:
            fh.write("y\n")
        assert ED._worktree_dirt_verdict(baseline, cwd, timeout=5) is True
    finally:
        ED._terminate_process_group(ancestor_proc.pid)
        ancestor_proc.wait(timeout=2)


def test_worktree_dirt_rename_out_of_excluded_root_reads_dirty(tmp_path, monkeypatch):
    wt, _main = _linked_worktree_pair(tmp_path)
    cwd = os.path.realpath(wt)
    leased_dir = os.path.join(cwd, "leased")
    os.makedirs(leased_dir, exist_ok=True)
    secret = os.path.join(leased_dir, "secret.txt")
    with open(secret, "w", encoding="utf-8") as fh:
        fh.write("secret\n")
    subprocess.run(
        ["git", "-C", cwd, "-c", "user.email=t@t.local", "-c", "user.name=t",
         "add", "leased/secret.txt"],
        check=True,
    )
    subprocess.run(
        ["git", "-C", cwd, "-c", "user.email=t@t.local", "-c", "user.name=t",
         "commit", "-qm", "add secret"],
        check=True,
    )
    ...
    assert baseline is not None
    subprocess.run(
        ["git", "-C", cwd, "mv", "leased/secret.txt", "escaped.txt"],
        check=True,
    )
    ...
    assert "escaped.txt" in porcelain
    assert "leased/secret.txt" in porcelain
    assert ED._worktree_dirt_verdict(baseline, cwd, timeout=5) is True


def test_worktree_dirt_rename_inside_excluded_root_still_excluded(tmp_path, monkeypatch):
    wt, _main = _linked_worktree_pair(tmp_path)
    cwd = os.path.realpath(wt)
    leased_dir = os.path.join(cwd, "leased")
    os.makedirs(leased_dir, exist_ok=True)
    with open(os.path.join(leased_dir, "a.txt"), "w", encoding="utf-8") as fh:
        fh.write("a\n")
    subprocess.run(
        ["git", "-C", cwd, "-c", "user.email=t@t.local", "-c", "user.name=t",
         "add", "leased/a.txt"],
    ...
    assert baseline is not None
    subprocess.run(
        ["git", "-C", cwd, "mv", "leased/a.txt", "leased/b.txt"],
        check=True,
    )
    assert ED._worktree_dirt_verdict(baseline, cwd, timeout=5) is False
...
@pytest.mark.parametrize("baseline_lease", ["none", "leased", "unreadable"])
@pytest.mark.parametrize("verdict_lease", ["none", "leased", "unreadable"])
def test_worktree_dirt_by_construction_dirty_outside_excluded_root_cross_product(
    tmp_path, monkeypatch, baseline_lease, verdict_lease,
):
    wt, main = _linked_worktree_pair(tmp_path)
    cwd = os.path.realpath(wt)
    ...
    try:
        baseline = ED._worktree_baseline(cwd, timeout=5)
        assert baseline is not None
        with open(os.path.join(cwd, "outside-dirty.txt"), "w", encoding="utf-8") as fh:
            fh.write("dirty\n")
        verdict = ED._worktree_dirt_verdict(baseline, cwd, timeout=5)
...
def test_worktree_baseline_overflow_producer_bound(tmp_path, monkeypatch):
    wt, _main = _linked_worktree_pair(tmp_path)
    cwd = os.path.realpath(wt)
    monkeypatch.setattr(ED, "MAX_BASELINE_ENTRIES", 3)
    for idx in range(5):
        with open(os.path.join(wt, "overflow-%d.txt" % idx), "w", encoding="utf-8") as fh:
            fh.write("%d\n" % idx)
    baseline = ED._worktree_baseline(cwd, timeout=5)
    assert baseline is not None
...
def test_parse_git_worktree_list_skips_pathless_block():
    porcelain = "\nbranch refs/heads/main\nworktree /tmp/real\n"
    worktrees = ED._parse_git_worktree_list(porcelain)
    assert len(worktrees) == 1
    assert worktrees[0]["path"] == os.path.realpath("/tmp/real")
...
# --- #609 stage 2: inseparable PR-body / session-dir staging ---


def _session_with_pr_body(tmp_path, content=b"PR body\n"):
    session = tmp_path / "session"
    session.mkdir()
    body = session / "pr-body.md"
    body.write_bytes(content)
    return str(session), str(body)


def test_dispatch_review_pr_body_unpaired_pr_only(tmp_path):
    repo_root = _repo(tmp_path)
...
def test_dispatch_review_pr_body_unpaired_session_only(tmp_path):
    repo_root = _repo(tmp_path)
...
def test_dispatch_review_pr_body_confinement_refusal(tmp_path):
    repo_root = _repo(tmp_path)
    outside = tmp_path / "outside"
    outside.mkdir()
    body = outside / "pr-body.md"
    body.write_text("outside session\n", encoding="utf-8")
    session = tmp_path / "session"
    session.mkdir()
    res = ED.dispatch_review(
        seat=_codex_seat(),
    ...
    assert res["detail"] == "sanitized-view-pr-body-outside-session"
    assert res["attempts"] == 0
    assert res["terminal"] is True


def _manual_open_review_run_with_pr_body(tmp_path, run_dir, *, pr_body_source):
    repo_root = _repo(tmp_path)
    build_view = _fake_build_view(tmp_path)
    view = build_view(os.path.realpath(repo_root))
    view["prBodyPath"] = _SV_MOD.PR_BODY_FILE_NAME
    view["prBodyBytes"] = 12
    cwd = os.path.realpath(view["path"])
    built = EA.build_argv_result(
        _codex_seat(), "review", {"model": "sonnet", "cwd": cwd},
    ...
    ok, detail = ED._open_review_run(
        ...
        fed_prompt=fed, order_id="test-order", progress_path=os.path.join(run_dir, "progress.jsonl"),
        repo_root=os.path.realpath(repo_root),
        pr_body_source_path=pr_body_source,
    )
    assert ok, detail
    return repo_root, view, pr_body_source


def test_continuation_pr_body_run_dir_mismatch_refused(tmp_path):
    run_dir = str(tmp_path / "run")
    session, body = _session_with_pr_body(tmp_path)
    repo_root, _view, journaled = _manual_open_review_run_with_pr_body(
        tmp_path, run_dir, pr_body_source=body,
    )
    other_body = os.path.join(session, "other.md")
...
def test_sanitized_view_receipt_includes_pr_body_keys():
    receipt = ED._sanitized_view_receipt(_fake_view_receipt(
        prBodyPath="SUPERHEROES_PR_BODY.md", prBodyBytes=42,
    ))
    assert receipt["prBodyPath"] == "SUPERHEROES_PR_BODY.md"
    assert receipt["prBodyBytes"] == 42


def test_grade_review_pr_body_payload_without_investigation_forfeit(tmp_path):
    run_dir, state = _grade_state_with_view_meta(
        tmp_path,
        {"prBodyPath": _SV_MOD.PR_BODY_FILE_NAME, "headSha": "abc"},
    )
    stdout = json.dumps({"findings": [{"id": "f1", "message": "issue"}]})
    with open(os.path.join(run_dir, "attempt-1.stdout"), "w", encoding="utf-8") as fh:
        fh.write(stdout)
...
def test_codex_native_review_spawn_argv_journals_single_output_flag(tmp_path):
    ...
    assert spawn_argv[o_idx + 1] == os.path.join(run_dir, ED.NATIVE_RESULT_NAME)
    assert "--json" in spawn_argv
    assert "--output-schema" in spawn_argv
...
def test_cursor_review_open_appends_marker_contract_byte_identical(tmp_path):
    ...
    records, _ = ED._journal_read(run_dir)
    opened = next(r for r in records if r.get("kind") == "run-opened")
    contract = EA.REVIEW_RESULT_CONTRACT(opened.get("expectedResultKind"))
    prompt_text = opened["fedPrompt"]
    assert prompt_text.endswith(contract)
