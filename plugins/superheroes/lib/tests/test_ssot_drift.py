...
# --- Cluster: stack_check refusal tokens (stack_check → merge-train.md) ----------

def _stack_check_reason_tokens_from_home():
    import stack_check

    reasons = {}
    for name in dir(stack_check):
        if not name.startswith("REASON_"):
            continue
        val = getattr(stack_check, name)
        assert isinstance(val, str) and val, (
            "stack_check.%s must be a non-empty str, got %r" % (name, val))
        reasons[name] = val
    assert reasons, "stack_check: no REASON_* constants discovered (rename/refactor?)"
    return reasons


def test_stack_check_reason_tokens_in_merge_train_doc():
    """§11: merge-train.md restates stack_check.py's REASON_* refusal tokens literally."""
    reasons = _stack_check_reason_tokens_from_home()
    doc = _read("skills/showrunner/reference/merge-train.md")
    missing = [name for name, token in sorted(reasons.items()) if token not in doc]
    assert not missing, (
        "merge-train.md missing refusal token(s) from stack_check.py: %r"
        % [(n, reasons[n]) for n in missing])


# --- Cluster: stack_check QUERY (stack_check → native-stacks.md) -----------------

def _normalize_stack_membership_query(query):
    normalized = query.replace("$first:Int!,", "")
    normalized = normalized.replace("entries(first:$first,", "entries(first:50,")
    return "".join(normalized.split())


def test_native_stacks_graphql_query_matches_stack_check():
    """§11: native-stacks.md's membership GraphQL block matches stack_check.QUERY modulo $first."""
    import stack_check

    doc = _read("rubric/native-stacks.md")
    match = re.search(r"```graphql\n(.*?)```", doc, re.DOTALL)
    assert match, "native-stacks.md: expected fenced graphql membership query block"
    doc_query = match.group(1).strip()
    code_query = stack_check.QUERY.strip()
    assert _normalize_stack_membership_query(code_query) == _normalize_stack_membership_query(doc_query), (
        "native-stacks.md membership query drifted from stack_check.QUERY "
        "(compare modulo $first parameterization)"
    )
