...
def test_recover_entry_emits_generation(tmp_path):
    # A real git repo + store so acquire succeeds; assert the JSON now carries "generation" and "root".
    ...
    out = subprocess.run([sys.executable, ENTRY, "--work-item", "wi", "--snapshot"],
    ...
    assert obj.get("root") == os.path.realpath(str(tmp_path))
