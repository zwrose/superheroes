...
# --- fold premise dependency field (#1340 layer 2f) --------------------------


def test_fold_dependency_from_well_formed_premise():
    rec = _reserved(
        "l1",
        "b",
        ["a"],
        "/tmp",
        premise={"dependency": 1340},
    )
    result = ll.fold([rec])
    assert result["ok"] is True
    lane = result["launches"]["l1"]
    assert "dependency" in lane
    assert lane["dependency"] == 1340


def test_fold_dependency_none_when_premise_omits_key():
    rec = _reserved("l1", "b", ["a"], "/tmp", premise={})
    result = ll.fold([rec])
    assert result["ok"] is True
    lane = result["launches"]["l1"]
    assert "dependency" in lane
    assert lane["dependency"] is None
...
def test_fold_dependency_none_when_malformed(dependency):
    rec = _reserved(
        "l1",
        "b",
        ["a"],
        "/tmp",
        premise={"dependency": dependency},
    )
    result = ll.fold([rec])
    assert result["ok"] is True
    lane = result["launches"]["l1"]
...
def test_fold_dependency_none_when_premise_absent():
    rec = _reserved("l1", "b", ["a"], "/tmp", premise=None)
    result = ll.fold([rec])
    assert result["ok"] is True
    lane = result["launches"]["l1"]
    assert "dependency" in lane
