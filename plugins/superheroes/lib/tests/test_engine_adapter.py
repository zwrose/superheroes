...
def test_engine_dispatch_timeout_expiry_contract_is_stated_in_dispatch_reference():
    # #202 / #623: harness 2.1.219 — foreground Bash converts at 600s; turn-end kills converted runs.
    # The supervised runner owns the bound (--max-wait below conversion, dispatch-poll). This reference
    # must keep the expiry contract: unreadable / forfeit → reviewer UFR-7, fixer falls open to Claude;
    # and must not regress to clamp-and-kill or hand-rolled perl dispatch as the sanctioned path.
    ref = os.path.join(_HERE, "..", "..", "skills", "review-code", "reference", "auto-fix-loop.md")
    with open(ref, encoding="utf-8") as fh:
        text = fh.read()
    ...
    assert "supervised runner" in text.lower()
