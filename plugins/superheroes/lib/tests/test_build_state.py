...
def test_parse_trailers_accepts_final_review_sentinel():
    # #375: whole-branch final-review fix commits carry the reserved sentinel Task-Id, which the gate
    # maps EVEN THOUGH it is not among the numeric task valid_ids. Without this the spine's own
    # final-review fix commits fail its own UFR-7 resume gate, every time, by construction.
    body = "build: fix whole-branch findings\n\nTask-Id: %s\n" % bs.FINAL_REVIEW_TASK_ID
    committed, unmapped = bs.parse_trailers([("sha", body)], {"1", "2"})
    assert unmapped == 0
    assert committed == [bs.FINAL_REVIEW_TASK_ID]


def test_parse_trailers_final_review_sentinel_maps_with_empty_valid_ids():
    # The sentinel is recognized on its own authority (the gate's reserved identity), not because it
    # happens to be in valid_ids — so it maps even when valid_ids is empty.
    body = "build: fix\n\nTask-Id: %s\n" % bs.FINAL_REVIEW_TASK_ID
    committed, unmapped = bs.parse_trailers([("sha", body)], set())
    assert unmapped == 0
    assert committed == [bs.FINAL_REVIEW_TASK_ID]


def test_parse_trailers_still_rejects_unknown_slug_id():
    # #375 keeps UFR-7 meaningful: the fix reserves ONE sentinel, it is NOT "accept any string".
    # A commit trailered with the work-item SLUG (the pre-fix external mint) is STILL unmapped — so a
    # hand commit with an arbitrary Task-Id cannot sail through, and old parked runs whose fix commits
    # carry the slug still fail closed until re-trailered to the sentinel (documented backward-compat).
    body = "build: fix\n\nTask-Id: my-work-item-slug\n"
    committed, unmapped = bs.parse_trailers([("sha", body)], {"1", "2"})
    assert unmapped == 1
    assert committed == []
