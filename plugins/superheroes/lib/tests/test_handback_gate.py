...
def _attested_receipt():
    return {
        ...
        "attestation": {"by": "owner", "reason": "manual"},
    }
...
def _scoped_repo(tmp_path):
    ...
    _write_sidecar(repo, session, _certified_receipt(), base_ref=base_sha)
...
def test_only_stale_marker_refuses(tmp_path):
    repo = _init_repo(tmp_path / "repo")
    _write_build_lane(repo, repoRoot="/stale")
    result = hg.validate_handback("gh pr ready", repo)
    assert result["decision"] == "refuse"
    assert result["reason"] == "handback-marker-stale"
...
def test_verdict_halted_refuses(tmp_path):
    ...
    session = str(tmp_path / "session")
    _write_build_lane(repo)
    _write_sidecar(repo, session, _certified_receipt(verdict="halted"), verdict="halted", base_ref=base_sha)
    result = hg.validate_handback("gh pr ready", repo)
...
def test_converged_without_certification_refuses(tmp_path):
    ...
    _write_build_lane(repo)
    _write_sidecar(repo, session, receipt, verdict="converged", base_ref=base_sha)
    result = hg.validate_handback("gh pr ready", repo)
    assert result["reason"] == "handback-verdict-not-allowlisted"
...
def test_attested_without_block_refuses(tmp_path):
    ...
    _write_build_lane(repo)
    _write_sidecar(repo, session, receipt, verdict=RD.ATTESTED_VERDICT, base_ref=base_sha)
    result = hg.validate_handback("gh pr ready", repo)
    assert result["reason"] == "handback-verdict-not-allowlisted"
...
def test_valid_attested_allows(tmp_path):
    ...
    session = str(tmp_path / "session")
    _write_build_lane(repo)
    _write_sidecar(repo, session, _attested_receipt(), verdict=RD.ATTESTED_VERDICT, base_ref=base_sha)
    result = hg.validate_handback("gh pr ready", repo)
    assert result["decision"] == "allow"
