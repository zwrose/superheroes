...
def _read_transcript_rows(config_dir, session_id):
...
def _write_native_result_payload(run_dir_real, attempt, payload_obj):
    """Write scrubbed payload to native result path with O_EXCL. Returns outcome token."""
    result_path = _native_result_path(run_dir_real, attempt)
    if result_path is None:
        return "error"
    if payload_obj is None:
        try:
            ...
            return "absent"
        return "occupied"
    scrubbed = _scrub_native_payload(payload_obj)
    payload = json.dumps(scrubbed, separators=(",", ":")) + "\n"
    try:
        fd = os.open(
            result_path,
...
def _run_engine_files_background(
    ...
    while _NOW() < deadline:
        rows, transcript_paths = _read_transcript_rows(config_dir, session_id)
        ...
        if engine_adapter.claude_transcript_turn_ended(rows):
            ...
            if payload is not None:
                transcript_result = _write_native_result_payload(
                    run_dir_real, attempt, payload,
                )
            else:
                transcript_result = _write_native_result_payload(
                    run_dir_real, attempt, None,
                )
            break
...
def _materialize_stdout_result(run_dir_real, attempt, opened, stdout_path, transcript_rows=None):
    """Materialize native result for stdout or transcript delivery. (#1273)"""
    try:
        delivery = engine_result_channel.result_delivery(
            opened.get("engine"), opened.get("claudeMode"),
        )
    ...
    if delivery == engine_result_channel.RESULT_DELIVERY_STDOUT:
        ...
        if (not isinstance(env, dict)
            ...
            try:
                ...
                return "absent"
            return "occupied"
        return _write_native_result_payload(run_dir_real, attempt, env["structured_output"])
    if delivery == engine_result_channel.RESULT_DELIVERY_TRANSCRIPT:
        if transcript_rows is None:
            return None
        payload = engine_adapter.claude_transcript_result(transcript_rows)
        return _write_native_result_payload(run_dir_real, attempt, payload)
    return None


def _stdout_delivery_gate(run_dir_real, attempt, opened):
    """Refuse admission when native delivery did not materialize a result. (#1273)"""
    try:
        delivery = engine_result_channel.result_delivery(
            opened.get("engine"), opened.get("claudeMode"),
        )
