...
def _run_engine_files(run_dir_real, attempt, argv, cwd, prompt_path, stdout_path,
    ...
    if coherence_err:
        ...
        return
    ok_prep, argv, native_result_path, stale_removed, prep_refusal = _spawn_native_result_argv(
...
def _grade_native_review_attempt(run_dir_real, attempt, opened, cwd, engagement, echo_nonce):
...
def _grade_review_attempt(run_dir_real, state, attempt):
    ...
    if _opened_channel(opened) == engine_result_channel.CHANNEL_NATIVE:
        return _grade_native_review_attempt(run_dir_real, attempt, opened, cwd, engagement, echo_nonce)
