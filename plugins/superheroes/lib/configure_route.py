...
def work_in_flight(cwd, *, root=None):
    ...
    under a storage-mode switch), so the conductor can warn before a storage-mode switch. v2 has
    no lease store — the spine's `ref_lock` (and its common-dir control-plane leases) was retired
    with the execution spine (#478) — so there is no machine-readable in-flight signal anymore.
    This fails OPEN (no known in-flight work): a warning is only ever a cost concern, never a
    safety one, and an honest "we don't know" is better than a fabricated lease check. Kept as a
    function (not deleted) because configure's prose still calls it before a storage-mode switch."""
    return False
