...
10. If `fix-batch` is empty (everything this round was skipped) → **EXIT** with a "remaining findings deliberately skipped" note (list them). (This all-skipped exit intentionally pre-empts the step-14 gate; `loop_state`'s `exit_skipped` arm is the revise loops' path via `--compiled`, not review-code's — review-code resolves the all-skipped case here.)
