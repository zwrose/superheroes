...
- **Process-level fail-closed.** The hook command is wrapped so a non-startable /
  ...
  fallback); the ⓪ **per-matcher canaries** (one through Bash, one through Write)
...
- **Parks-safely DECISION (pytest).** `reset.plan_reset` GATEs on a live lock or an
  ...
  behavior (SKILL ⓪ / "park safely"), exercised in the dogfood — not a unit invariant.

## Seeded-failure arms (fixtures)

- review `exit_skipped` / `halt` → GATE (the deterministic ② read).
- test-pilot failure → fix/GATE.
- CI red unfixable / loop exhausted → revert-to-draft + GATE.
- **Library/CLI applicability arm** — the ④⑤⑥-skip path (the arm THIS repo's own
  changes take: no runnable surface → ②③⑦⑧⑨ only).
