...
- **[live] Identifiers via the reference impls.** The work-item slug and the work-branch
  content-hash are produced by [`lib/identifiers.py`](../plugins/superheroes/lib/identifiers.py) (or a vendored
...
- **[Phase 1 — entry-gate] Content-hash canon-versioning.** When `content_hash` gets its
  ...
  (Confirm this decision when `content_hash` gets its first consumer.)
