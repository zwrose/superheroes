...
| --- | --- | --- | --- | --- |
| **0 · Foundations** *(current)* | a clean brand + the contracts everything builds on | rebrand to `superheroes`; [CONVENTIONS.md](CONVENTIONS.md) (calibration, define-docs, state tiers, disk-state); the eval-harness skeleton | review-crew, test-pilot | — |
| **1 · One issue, supervised** | prove the full loop on a single work-item, with a human watching | one work-item taken Discovery→Integrate end-to-end; evals gate every step | + **define** (spec/plan/tasks), the **review trio** (review-spec/plan/tasks) | — |
...
| **2b · Define depth** | the front half gets serious: real design + recursion | richer Plan + the full review trio; recursive human-approved decomposition; living calibration that evolves with the project | define, review-crew (deepened) | — |
