...
canonical IDs (plus `gpt-6-astra` for `reviewer-deep` only, and only after its registration probe
has passed — until then the pin is refused `pin-probe-pending`); a pinned model runs at the effort
