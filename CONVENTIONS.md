...
`core_md`'s engine-preference write gate, `model_tier_overrides`' tier-writer gate, the
preflight `dispatch-vocab` probe, and `core_md.migrate_on_read` read the project's `core.md`
...
`core-md-unreadable` rather than treated as absent (migration **defers** and preserves both
`core.md` and the legacy profile instead of refusing at configure time); a genuinely absent
